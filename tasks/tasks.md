...
> **⚠ MERGE PENDING — `origin/main` advanced while the correction pass was in flight (designer's multi-animal NI work, to `5b20ffd`).** Reviewed 2026-06-12; see **[`origin_main_merge_review.md`](origin_main_merge_review.md)**. The designer added a restart registry schema (`sample_organism` + `subject_id` + `anatomical_entity`, commit `23df602`) that **supersedes the correction-pass S1** (`subject_id` only) — same column, theirs is the superset; on merge, adopt theirs. They also independently fixed the cross-drive relpath (= item 7) and added the **multi-animal NI solution**: one scan = one entry recording its 1–4 animals as a list, with a new top-level **`registry_subjects.csv`** subjects table. **Open design decision (NI-LIVE-08):** how the singular `subject_id` column represents a multi-animal scan's N>1 — recommended: keep `subject_id` singular (= primary) + the subjects table for the one-to-many + sidecar `subjects:[…]`, **no** plural `subject_ids` column (the doc's `subject_ids` is a non-key convenience pointer). Merge is NOT executed — awaiting the NI-LIVE-08 call + go-ahead (§4 of the review doc).
