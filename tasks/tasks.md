...
> **▶ ACTIVE PASS (2026-06-03): Phase 3 — preclinical metadata enrichment writer.** Wiring the `subject:`/`condition:`/`anatomy:` blocks into ingest (non-blocking model). Specs locked; `animal_db.py` fetcher built + verified. **The one consolidated build thread is [`tasks/phase3_metadata_enrichment_plan.md`](phase3_metadata_enrichment_plan.md)** — start there for the code work. Detail trail in §3.2.
