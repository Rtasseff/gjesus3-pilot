...
- [ ] **FID→DICOM regeneration for no-DICOM internal MRI acquisitions** (UPDATED 2026-05-27 round-6 v2; was originally an assessment task in v1). When students don't run Bruker's GUI DICOM exporter, the acquisition's `pdata/<idx>/dicom/` is missing entirely — raw `fid` is present but no analysis-ready DICOM. Round-6 v2 round detected 3 of 7 source projects in this state. Currently the ingest registers these as empty-`.data/` placeholders. **The capability we want:** a `fid_to_dicom.py` module that reads the binary `fid` + JCAMP-DX aux (acqp/method/visu_pars/subject) and emits conformant DICOM directly, no Bruker GUI required. **Why it's not done yet:** No open-source Python library does this end-to-end. `bruker2nifti`/`brkraw` go to NIfTI not DICOM; `dcm2niix` runs the opposite direction; Bruker's own exporter is closed-source/GUI-only. Building it requires (a) binary FID reader (per-version: ParaVision 3.6/6/7 differ); (b) Fourier reconstruction (or rely on `2dseq` if present + take that as the post-recon binary); (c) DICOM construction via pydicom with full MRI-specific tag population (MagneticFieldStrength/EchoTime/RepetitionTime/FlipAngle/etc. mapped from JCAMP-DX); (d) validation against Bruker's own export to ensure interchangeable output. Estimated 2-4 weeks for a working prototype, +2-3 weeks for hardening. Track as an independent stream when capacity allows. When it lands: backfills empty-`.data/` placeholders via idempotent re-ingest with no other code changes.
...
- [ ] **Study-metadata work stream** (architecture in [08_METADATA §1](../mfb-rdm-docs/08_METADATA.md), 2026-05-12). Order is roughly: gather → import → close-out. None blocks the first user-driven Phase B ingest.
  ...
  - [ ] **Animal-facility-DB integration → preclinical `subject:` block auto-population** (NEW 2026-05-29). Spec landed in [08_METADATA §4.4](../mfb-rdm-docs/08_METADATA.md) — 4 required fields (species / strain / sex / age_at_acquisition) DECIDED, optional fields + `source:` tag DRAFT. Required for `sample_type ∈ {organism, tissue}` — always for internal MRI + NI, typically for animal-derived microscopy. Without it the acquisition is significantly less valuable for publication / reuse. **4-phase work:**
    - [ ] **Phase 1 — Scope the animal-facility-DB API.** Confirm DB system + query interface (REST? SQL? CSV export?), what fields it exposes per animal, auth model. **BLOCKED on IT** for programmatic access — request open with the IT team that's also working the QNAP permissions cleanup. Output: a one-page note documenting endpoint + auth + field-to-`subject:`-block mapping.
    - [ ] **Phase 2 — Write `tools/animal_db.py` fetcher.** Function `lookup(facility_animal_id) -> dict` returning the populated `subject:` block (or empty dict + log warning if animal not in DB). Cache locally (idempotent re-runs shouldn't hammer the DB). Credentials from env var / config file outside repo, same pattern as `ftp_mirror.py`. Standalone unit test against a couple of known animal IDs.
    - [ ] **Phase 3 — Wire into ingest.** New YAML field `auto_discover.subject_from_db: true` (default false). When set, `metadata_sidecar.py` calls `animal_db.lookup(<facility_animal_id from registry sample_id or discovered field>)` and writes the result to `sidecar["subject"]` with `source: "animal-facility-db"`. Operator override path: explicit `subject:` block in YAML config beats DB lookup (set `source: "operator-entered"`). Documented in [10_TOOLS](../mfb-rdm-docs/10_TOOLS.md) once schema settles.
    - [ ] **Phase 4 — Backfill existing acquisitions.** Round-6 v2 MRI (97 acqs) + Round-8 v2.1 NI (84 acqs) + any animal-derived microscopy (AxioScan 28 + Cell Observer 165 + LSM 900 13 — to the extent samples are animal-derived) currently have no `subject:` block. Two paths: (a) idempotent re-ingest after Phase 3 lands — re-walks staging, re-writes sidecar; (b) standalone `subject_backfill.py` that walks `/raw/` and adds the `subject:` block in-place (controlled write to `/raw/` like the project close-out tool — coordinate with the raw-immutability lockdown design §4.3). Path (a) preferred if staging is still around; (b) for acqs whose staging is gone.
    - [ ] **Future enhancement:** add `subject:` field validation to `validate_registries` (Phase 4 of §3.2) — flag rows with `sample_type ∈ {organism, tissue}` whose sidecar's `subject:` block is missing or has unpopulated required fields. Surfaces the gap during the quasi-production-to-true-production transition.
