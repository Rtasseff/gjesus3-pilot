...
**Last Updated:** 2026-06-23 — researcher Finder **enhancements built** at the data office's request on the still-ON-HOLD branch `feat/finder-mvp` (auto-refresh of `index.html` on ingest; expanded columns incl. project `short_name` + owner; sidecar copy-path/metadata view; `DEFAULT_LINK_BASE` → container UNC) — **pending the user's review then merge, NOT merged** (see Section 0). Prior: 2026-06-18 — Finder MVP built, pushed + global index published, then put ON HOLD pending a value/complexity evaluation. Full dated history in [CHANGELOG.md](../CHANGELOG.md).
...
> **▶ RESEARCHER FINDER — ⏸ ON HOLD (2026-06-18) — MVP BUILT + GLOBAL INDEX PUBLISHED; branch `feat/finder-mvp` PUSHED to `origin/feat/finder-mvp` (in sync), NOT merged to main.** **Parked pending a value/complexity evaluation — very basic (copy-paths only) but a starting place; focus moved to the NI live-machine sync ingest (below).** A searchable, self-contained HTML index of the registry so researchers find their own data over SMB (no XNAT/OMERO, no server, no Python on their side). Two pieces over one join (`registry_raw` ⋈ `registry_projects`): **`tools/find_acq.py`** (join engine + CLI) and **`tools/generate_index.py`** (writes the self-contained `index.html` — data embedded inline as JSON, vanilla JS, **Copy-path** buttons because browsers block `file://`→`file://` navigation). Doc: [`tools/FINDER.md`](../tools/FINDER.md); tests `tools/test_find_acq.py` (all pass). Commit `875307e`.
> - **✅ Global index PUBLISHED + verified on the live share** at `J:\gjesus3-data\registries\index.html` (~15 MB, ~13.5k acqs). Generated with `--link-base 'J:\gjesus3-data'`; verified the embedded copy-paths read `J:\gjesus3-data\raw\…`. ⚠️ The `\\GJESUS3\gjesus3` UNC (the script's `DEFAULT_LINK_BASE`) does **not** resolve from this session — an earlier preview using it had wrong paths; corrected. This `index.html` is a **generated artifact, not in git** — reproduce/refresh with `PYTHONPATH=tools python tools/generate_index.py --nas-root J:/gjesus3-data --link-base 'J:\gjesus3-data'`.
> - **🆕 ENHANCEMENTS BUILT 2026-06-23 (data office's request; branch `feat/finder-mvp`, PENDING the user's review → then merge — NOT merged):** (1) **auto-refresh on ingest** — a successful (non-dry-run) `ingest_raw.py` batch now regenerates `registries/index.html` non-fatally (refresh failure → WARN, never fails the ingest); the Finder index is no longer a manual step (manual `generate_index.py` still works); (2) **expanded table columns** (in order: Acq ID, Date, Instrument, Modality, Researcher, Operator, Sample, Subject, Organism, Sample type, Original name, **Project = project `short_name`**, **Owner = project owner**); (3) **detail panel** adds **File size (MB)** + **Project description**, with **Copy-path buttons** for the metadata / raw / project paths (pasting the `metadata.json` path into Chrome/Edge renders the sidecar as a native collapsible JSON tree); (4) **`DEFAULT_LINK_BASE` fix** to the portable container UNC `\\GJESUS3\gjesus3\gjesus3-data` (the old default omitted the `gjesus3-data` component → unresolvable paths). Docs updated: [`tools/FINDER.md`](../tools/FINDER.md), [06_REGISTRIES §1.2](../mfb-rdm-docs/06_REGISTRIES.md) (`index.html` = generated artifact), `10_TOOLS.md`/`INGEST_CLI.md` (auto-refresh step), `00_INDEX.md` + `CHANGELOG.md`.
> - **⏳ Awaiting the user (3 open decisions):** (1) **confirm researcher machines map the NAS as `J:`** (Ryan's does) — the generator default is now the portable container UNC `\\GJESUS3\gjesus3\gjesus3-data`, so a default-`--link-base` index resolves on any machine; pass a `J:\gjesus3-data` base only if you know every researcher maps that letter; (2) **publish the per-project indexes too?** (`--per-project` writes an `index.html` into every project folder — built + tested; held off because each exposes that project's acqs to anyone who can read the folder); (3) **evaluate MVP value/complexity**, then decide whether to **merge `feat/finder-mvp` → main + push** and which remaining *Next* features (saved/shareable searches, CSV/methods-section export, counts dashboard, per-row XNAT/OMERO launch links). Product framing + scope/limits in [`tools/FINDER.md`](../tools/FINDER.md).
