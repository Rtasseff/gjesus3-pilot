...
>
> **▶ RESEARCHER FINDER — ✅ ENHANCEMENTS MERGED TO MAIN (2026-06-23).** The searchable, self-contained HTML index of the registry (`registries/index.html` — researchers double-click it over SMB; no XNAT/OMERO, no server). Built as an MVP (branch `feat/finder-mvp`), then enhanced + **merged to `main`** at the data office's request: (1) **auto-refresh on ingest** — a successful (non-dry-run) `ingest_raw.py` batch regenerates `registries/index.html` non-fatally (refresh failure → WARN, never fails the ingest); manual `generate_index.py` still works; (2) **expanded columns** (Acq ID, Date, Instrument, Modality, Researcher, Operator, Sample, Subject, Organism, Sample type, Original name, **Project** = `short_name`, **Owner**); (3) **detail panel** adds **File size (MB)** + **Project description**, with **Copy-path** buttons for the metadata / raw / project paths (paste the `metadata.json` path into Chrome/Edge for a native collapsible JSON tree); (4) **`DEFAULT_LINK_BASE` fix** → portable container UNC `\\GJESUS3\gjesus3\gjesus3-data`. Pieces: `tools/find_acq.py` (join + CLI) and `tools/generate_index.py`; doc [`tools/FINDER.md`](../tools/FINDER.md); tests `tools/test_find_acq.py` (all pass).
> - **⏳ Still open (non-blocking):** (1) **confirm researcher machines resolve the UNC `\\GJESUS3\gjesus3\gjesus3-data`** (the new default `--link-base`) — else regen with a `J:\gjesus3-data` drive-letter base; (2) **publish per-project indexes?** (`--per-project` writes an `index.html` into each project folder — built + tested; held off because each exposes that project's acqs to anyone who can read the folder); (3) remaining *Next* features (saved/shareable searches, CSV/methods export, counts dashboard, per-row XNAT/OMERO launch links). The "select-in-Finder → assemble a project" idea is logged in [`BACKLOG.md`](BACKLOG.md).
>
> ---
