...
> **▶ CURRENT (2026-06-11) — correction pass landed, on branch `fix/correction-pass-2026-06` (NOT yet merged to `main`).** The post-first-review correction pass is implemented + sandbox-verified (plan: [`correction_pass_plan.md`](correction_pass_plan.md) + [`correction_pass_handoff.md`](correction_pass_handoff.md); full list in [CHANGELOG.md](../CHANGELOG.md) 2026-06-11). **Correctness/integrity (gates the historical-data ingest):** registry lock + durable ACQ-ID reservation + BOM/trailing-newline-safe CSV appends (new `tools/ingest/{locking,csv_safe}.py`, [06_REGISTRIES §2.7](../mfb-rdm-docs/06_REGISTRIES.md)); archive re-run idempotency; crash-after-copy rollback (registry append = commit point; `--delete-source` moved to the end); empty-folder fail-loud; cross-drive relpath guard. **Schema:** auto `subject_id` registry column (S1, migration run on sandbox; live registry migrate pending sign-off); in-use conventions promoted DRAFT→DECIDED (S2); suggested vocabularies (S3, [08_METADATA §4.8](../mfb-rdm-docs/08_METADATA.md)). **Operator:** GUI dry-run defaults ON with a loud banner + summary; CELL + LSM9 seed recipes; the GUI `.exe` builds + the frozen-path bug fixed (verified launch + recipes). **Docs:** `START_HERE.md` / `GLOSSARY.md` / `CHANGELOG.md`; header-blob de-bloat; stale cross-ref + DB-"blocked-on-IT" fixes.
> **⚠ MERGE PENDING — `origin/main` advanced while the correction pass was in flight (designer's multi-animal NI work, to `5b20ffd`).** Reviewed 2026-06-12; see **[`origin_main_merge_review.md`](origin_main_merge_review.md)**. The designer added a restart registry schema (`sample_organism` + `subject_id` + `anatomical_entity`, commit `23df602`) that **supersedes the correction-pass S1** (`subject_id` only) — same column, theirs is the superset; on merge, adopt theirs. They also independently fixed the cross-drive relpath (= item 7) and added the **multi-animal NI solution**: one scan = one entry recording its 1–4 animals as a list, with a new top-level **`registry_subjects.csv`** subjects table. **NI-LIVE-08 DECIDED 2026-06-12 (user):** the acquisition registry's singular `subject_id` (designer's + S1) becomes a **packed `subject_ids`** column (`;`-joined, always-a-list) — the scan→animal link + visible multiplicity; `registry_subjects.csv` is a **true one-row-per-subject table** (the facts we can grab; updatable; static-vs-dynamic deferred); sidecar carries a **minimal** subject subset; the data-office deferred-recovery path (`pending_subject_metadata.csv` → `recover_subject_metadata.py`) is the immutability "cheat" (already built); **no mapping/junction table now** (a deferred query-layer option). `sample_type`/`sample_id`/GUIs untouched. Merge NOT yet executed — design is set; coordinating who applies it (agent-merge vs designer-first). Full record: [`origin_main_merge_review.md`](origin_main_merge_review.md) §3–§4.
> **Awaiting the user (external deps, surfaced not blocking):** (1) a representative `.czi` to verify the frozen GUI's end-to-end metadata read; (2) run `tools/diagnostics/test_oslink.py` on the **MRI (Linux)** + **NI (Mac-fronted)** mounts and report the verdict so the hard-link-vs-fallback path is chosen per instrument; (3) sign-off to run the schema migration + a smoke ingest against the live `J:\gjesus3-data` registry; (4) optional `.my.cnf` install on a trusted machine to enable DB auto-fill ([tools/operator/README.md](../tools/operator/README.md)); (5) **push** `fix/correction-pass-2026-06` interactively (agent push hangs on the GCM dialog). Detail + the merge plan: [`origin_main_merge_review.md`](origin_main_merge_review.md).

> **▶ PREVIOUS (2026-06-09) — operator hand-off, on branch `operator-ingest-tooling` (NOT yet merged to `main`).** The NI + MRI **command-line tools are DONE and user-verified end-to-end** (real ingests run + accepted). **Next: test the microscopy GUI** (built + headless-verified; full click-through needs a real `.czi` from `S:`/`K:` — test plan in [`tools/operator/TESTING.md`](../tools/operator/TESTING.md)). This session also landed: **the global person/role rename** (registry `operator`→`researcher`; `operator` sidecar-only; permission roles Operator→Tech, User→Researcher; "user"=software only — [06_REGISTRIES §2.3a-bis](../mfb-rdm-docs/06_REGISTRIES.md), live registry header migrated); **`mri-ingest --operator` REQUIRED**; NI PI fix + MRI `pi_initials`/`sample_id` alignment + `pi_group_lookup.yaml`; condition/anatomy CLI flags + prompts; anatomy extended to tissue; condition for cells. Small follow-ups + the GUI "researcher REQUIRED" item are in [`BACKLOG.md`](BACKLOG.md). Detail memory: `[[operator-ingest-tooling]]` + `[[role_terminology]]`.

> **▶ GUI LAUNCH CHECKLIST (pre-launch — these gate operators using the GUI on real machines; NOT backlog).** As of 2026-06-10 the microscopy GUI saves operator recipes as **YAML to a configurable folder defaulting to `<NAS>/recipes`** (follows the Destination NAS) instead of the repo `tools/operator/recipes/`. Two operational steps must happen before operators are turned loose:
> - [ ] **Grant write access to the NAS `recipes/` folder.** Operators must be able to create/edit recipes under `<NAS>/recipes` (e.g. `J:\gjesus3-data\recipes`). One-time ACL grant on that folder using the existing `CICBIOMAGUNE\GJesus` group (Modify) — same pattern as `projects\`/`staging\`; IT won't make new groups. Without it, **Save recipe fails** on the NAS. (Ties into the §4.3 permission model.)
> - [ ] **Migrate the existing repo recipes onto the NAS.** Copy `tools/operator/recipes/*.json` (or re-save them via the builder as YAML) into `<NAS>/recipes`, so operators without the repo checkout see the existing conventions. The GUI still reads the repo dir as a read-only *seed* source, but only the data-office machine has it.
