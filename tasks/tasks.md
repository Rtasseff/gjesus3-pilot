...
**Last Updated:** 2026-06-24 — OPERATOR GUI ROLLOUT pass (microscopy + MRI one-click Windows apps); see §0 top. Prior: 2026-06-23 — researcher Finder enhancements merged to `main` (auto-refresh of `index.html` on ingest; expanded columns; sidecar copy-path/metadata view; `DEFAULT_LINK_BASE` → container UNC). Full dated history in [CHANGELOG.md](../CHANGELOG.md).

This file consolidates all open and completed tasks. Completed items are kept for context but marked with ~~strikethrough~~.

> **`tasks.md` vs [`BACKLOG.md`](BACKLOG.md) (split started 2026-06-09).** This file tracks the **work to get the pilot to users/operators** (the active delivery path). **[`BACKLOG.md`](BACKLOG.md)** holds **later improvements** — refinements and second-/third-stage features that aren't required for the hand-off. Promote a backlog item here when it becomes a delivery blocker.

---

## 0. Active Pass / Up Next

> **▶ ACTIVE PASS (2026-06-24) — OPERATOR GUI ROLLOUT (microscopy + MRI), branch `feat/mri-operator`.** Rolling the two browser-GUI ingest tools out to operators as one-click Windows apps. **One Flask app, two pages:** `/` = microscopy (recipes/builder, unchanged), `/mri` = the new simple MRI page (SFTP pull from the scanner → preview → ingest; NO recipes/builder). Launch the MRI page directly with the `--mri` flag. The MRI work is 8 commits on `feat/mri-operator` (`41feb20`→`af55fc4`), which sits on `feat/ni-live-sync` (4 commits = the `registry_subjects.csv` / subjects foundation the MRI ingest writes to). **Design choices made this pass — DOCUMENT in `mfb-rdm-docs`:**
> - **SFTP source is READ-ONLY** — only `listdir_attr`/`stat`/`get` are ever called against `kenia` (audited 2026-06-24; zero `put`/`remove`/`mkdir`/`rename`/write paths anywhere). Optional server-side hardening: ask the platform manager to set `mriuser` read-only.
> - **Creds** from `~/.ssh/gjesus3_mri.cred` (INI; `GJESUS3_FTP_*` env vars override). Per-machine deploy.
> - **Staging** = `<NAS>/staging/<auto-batch>`, **auto-deleted after a clean real ingest** (guarded: never a NAS top-level dir / drive root). Operator never sees a staging path.
> - **`instrument_model` auto-derived** from `acqp ACQ_station` ("Biospec 70/30"→7T, "117/16"→11.7T; 1H-freq fallback) — no operator field. `--model` still overrides.
> - Link-collision + on-NAS-overwrite warnings; default-on Dicomifier regen (graceful WARN + empty `.data/` placeholder when absent); **destination NAS validated BEFORE the pull**; non-scan ParaVision sibling folders skipped with a clean message.
>
> **▶ ROLLOUT STEPS (HIGH PRIORITY — work through in order):**
> 1. [x] **Default NAS already correct** — `GUI_DEFAULT_NAS_ROOT = \\gjesus3\gjesus3\gjesus3-data` (both pages; fresh installs get it automatically). Ryan's stale personal saved override (`%LOCALAPPDATA%\gjesus3-operator\nas_root.txt`) reset for parity.
> 2. [ ] **Merge to main** — a clean FAST-FORWARD (main HEAD == merge-base `96f714d`; branches are linear). **User-approved 2026-06-24 to land the `feat/ni-live-sync` subjects foundation too** — it's already production-proven (it created the live NI + MRI data on 2026-06-14; main is simply behind). Order: `feat/ni-live-sync` → main, then `feat/mri-operator` → main, then push `origin/main`.
> 3. [ ] **Freeze for Windows (one-click .exe)** — update `tools/operator/gui/microscopy_ingest.spec` (currently microscopy-only): **add `paramiko` (+`cryptography`,`bcrypt`,`pynacl`,`cffi`) to `hiddenimports`** (app.py imports paramiko LAZILY → PyInstaller's static analysis misses it → the frozen "Pull" would silently fail); **add `tools/ftp_mirror.py` to `datas`** (loaded by path via `_load_ftp_mirror`, not currently bundled). `mri.html`/`mri.js` already bundled (spec globs `templates/`+`static/`). ONE exe serves both pages. Build on a Windows box; **SMOKE-TEST THE FROZEN EXE** on a clean machine (lazy imports + the dynamic `_loader` core load are exactly what break under PyInstaller).
> 4. [ ] **Distribute + HTML docs** — exe + HTML user guides go in a NEW top-level dir at **`\\gjesus3\gjesus3\gjesus3-data\tools\`** (user choice 2026-06-24; the NAS has no `tools/` dir yet). One exe + **TWO desktop shortcuts** (Microscopy = exe; MRI = exe `--mri`). HTML not MD (novice operators); wire a **Help link in each GUI page**. **MRI per-machine deploy step: drop `~/.ssh/gjesus3_mri.cred`** (password pasted out-of-band) — without it the MRI page can't do anything (no local-source fallback). MRI operators = researchers at their own workstations, NOT the microscope machines. Dicomifier absent on operator machines → no-DICOM exams placeholder, regenerated later on a Dicomifier box.
> 5. [ ] **User test** — frozen exe on a clean (no-Python) machine first, then a 1–2 friendly-operator pilot per tool. MRI test machine needs creds + `kenia` reachability + NAS.
>
> **▶ ALSO document the design choices in `mfb-rdm-docs`** (10_TOOLS operator-GUI/SFTP-source section + read-only model + scanner-model auto-derive; bump 00_INDEX version history + Last Updated). Partly done: 09_MODALITIES + 08_METADATA already carry `mri_station`/`mri_scanner_model`.
>
> **▶ WHERE WE ARE (2026-06-24):** executing Steps 1–2 this session (Step 1 done; merge in progress). Next: Step 3 (spec update + frozen-exe smoke test). MRI GUI is fully built, live-tested end-to-end (pull from `kenia` → ingest → sandbox), and operator-feedback-trimmed.
>

> **▶ ACTIVE PASS (2026-06-19) — NI live-machine sync, branch `feat/ni-live-sync`.** Reactivated the live-box sync build — the essential forward path for live NI project data ("if we cannot sync live data we are done"). Goal: code the user can test on the live NI Mac (REMIW11) next week. **Step 2a landed (the subjects table):** `tools/ingest/subjects_table.py` — the one-row-per-SUBJECT `registry_subjects.csv` upsert (atomic temp-replace + registry lock + BOM-safe header check; merge keeps the higher-confidence record and never clobbers a DB row with a later `pending-db` stub; gaps fill regardless of source; idempotent re-run = no write). Wired into `ingest_raw.py` Step 10 (non-blocking, shares the lock — `_hold_lock=False`). Tests `tools/test_subjects_table.py` (33 checks, green; no DB, no NAS). Schema = `facility_id`(PK)·animal_code·project_alias·species·strain·sex·date_of_birth·genotype·cohort_id·source·first_registered·last_updated; this is the one-row-per-subject record, **not** a junction table (NI-LIVE-08; [06_REGISTRIES §2.3.2]). **⚠️ Verified the live `J:\gjesus3-data` has NO `registry_subjects.csv`** — the writer never existed before now, so the 13,555 historical acqs have packed `subject_ids` on `registry_raw` but no subjects table. **✅ Back-fill DONE (2026-06-19):** `tools/backfill_subjects_table.py` (reuses `row_from_subject_block` + `plan_upserts`/`upsert_subjects`; reads the cached sidecar `subject:` blocks → no live DB needed) wrote **715 distinct subjects** to live `J:\gjesus3-data\registries\registry_subjects.csv` (687 `animal-facility-db` + 28 `pending-db`; 0 missing/unreadable sidecars; verified one-row-per-subject, no dup ids; re-run = no-op). **✅ Live sync DONE (2026-06-20) — it's a config, not an orchestrator.** Proved the existing `ingest_raw` already does the whole walk: recursive `path_parse` discovery + the dedup index (ingest only what's new) + `copy_ni_acquisition`/`ni_metadata`/registry/subjects/project-link — `expand_batch` discovers live anchors with ZERO new ingest code. The only new code (multi-animal, NI-LIVE-08): a `subject_parse` discovery hook (`config.py`, reuses `ni_live_discover.parse_subject`) splitting the messy subject folder → project + 1-4 animals; `enrichment._build_subjects` resolving the 1-4 via the SAME single-animal DB path (returns a subjects LIST, single = length-1); `ingest_raw` threading it → packed `subject_ids` + sidecar `subjects:[]` + one subjects-table row per animal. Config: `tools/templates/instruments/molecubes_ni_live.yaml`. Verified end-to-end (synthetic box + injected DB + real `ingest_raw --dry-run`: `0324_m59_m60` → `subject_ids 59-..;60-..`, single-animal unchanged, full test sweep green). **Run per researcher:** `python tools/ni_live_discover.py --root <box>/<user> <user>` (eyeball the flagged messy names), then `python tools/ingest_raw.py --config tools/configs/<copy>.yaml --nas-root J:/gjesus3-sandbox`. **Gate-0** (`os.link` on the Mac CIFS mount) still pending the user — gates a real live ingest, not the build/sandbox.
>

> **▶ CURRENT STATE — full historical imaging archive in TRUE PRODUCTION (2026-06-18). Nothing in flight; safe to restart.** All on-network historical imaging is ingested into `J:\gjesus3-data` — **~13,555 acquisitions**, all checksummed + sidecar'd, with subjects + derived/best-guess anatomy where available. Everything below this banner is committed + pushed (`origin/main` @ `e335654`); the durable per-instrument record is [`equipment/historical_data_archives.md`](../equipment/historical_data_archives.md) + session memory.
> - **MRI 10,314** — 6,405 DICOM-bearing + **3,909 no-DICOM regenerated** via Dicomifier (from WSL; configs `tools/configs/mri_jrc_{animalfirst,projfirst}_regen.yaml`); project hard-links created by the Windows pass `tools/relink_mri_regen.py`; **anatomy back-filled 5,217** (`tools/backfill_mri_anatomy.py` + the one-time Jesús `Cine_IG_FLASH→heart` override `tools/configs/mri_anatomy_override_2026-06.yaml`).
> - **AxioScan 7 (ZWSI) 565** — MFB-filtered; anatomy 412 from the sample-id organ suffix (`tools/backfill_microscopy_anatomy.py`); 4 `CTRL` `is_control:true`; link-name collisions date-disambiguated (`tools/relink_axioscan_collisions.py`).
> - **Cell Observer (CELL) 1,739 + Confocal LSM 900 (LSM9) 805** — LEGACY **best-guess** ingest (no naming standard): per-folder configs via `tools/gen_microscopy_bestguess_configs.py`; off-NAS guess pass `tools/backfill_microscopy_bestguess.py` + `tools/reference/microscopy_bestguess_map.yaml` (`source=auto-guess`). CELL: 1,138 cells / 146 tissue / 455 unknown. LSM9: 223 cells / 582 unknown.
> - **Nuclear imaging (PET 66 + CT 66) = 132** — gnuclear2$ 2025/Jesus archive preload.
> - **Designer's 2026-06 code-review correction pass PULLED + verified** (fast-forward to `e335654`, in sync). MRI slim-copy confirmed **collision-free** (overwrite impossible by-construction — `MRIm<NN>`→`recon<idx>_frame<NN>` is per-recon-unique — + a 486-acq sample: 0 gaps, 0 count mismatches).
>
> **▶ UP NEXT (all in [`BACKLOG.md`](BACKLOG.md), none blocking).** (1) **External-drive microscopy** — the larger future effort; the best-guess system runs verbatim there. (2) **Researcher feedback → re-project** the best-guess microscopy (esp. Cell Observer's per-PERSON projects = a deliberate STOPGAP; `original_name` preserves source paths → cheap re-home) + push better project naming going forward. (3) Minor: strip `.czi` from best-guess `sample_id`; the **`-None` subject back-fill** (pending the facility populating null project aliases for 0219/0618/0619/1521); MRI/AxioScan link-naming refinements; ~360 MRI spectroscopy (STEAM/PRESS) separate path; ~165 NI/MRI `pending-db` subject recoveries.
>
> **▶ RESEARCHER FINDER — ✅ ENHANCEMENTS MERGED TO MAIN (2026-06-23).** The searchable, self-contained HTML index of the registry (`registries/index.html` — researchers double-click it over SMB; no XNAT/OMERO, no server). Built as an MVP (branch `feat/finder-mvp`), then enhanced + **merged to `main`** at the data office's request: (1) **auto-refresh on ingest** — a successful (non-dry-run) `ingest_raw.py` batch regenerates the global `registries/index.html` **+ every per-project `index.html`** non-fatally (refresh failure → WARN, never fails the ingest); manual `generate_index.py` still works; (2) **expanded columns** (Acq ID, Date, Instrument, Modality, Researcher, Operator, Sample, Subject, Organism, Sample type, Original name, **Project** = `short_name`, **Owner**); (3) **detail panel** adds **File size (MB)** + **Project description**, with **Copy-path** buttons for the metadata / raw / project paths (paste the `metadata.json` path into Chrome/Edge for a native collapsible JSON tree); (4) **`DEFAULT_LINK_BASE` fix** → portable container UNC `\\GJESUS3\gjesus3\gjesus3-data`. Pieces: `tools/find_acq.py` (join + CLI) and `tools/generate_index.py`; doc [`tools/FINDER.md`](../tools/FINDER.md); tests `tools/test_find_acq.py` (all pass).
> - **✅ Per-project indexes NOW ENABLED (2026-06-23):** `--per-project` is wired into the ingest auto-refresh, so each project folder gets its own scoped `index.html` (filtered by `project_hint`, regenerated on every successful ingest). Caveat retained: a project's index lists its acqs to anyone who can read that folder — fine for the single group today; revisit under per-project ACLs. A **provenance-driven** project index (a possibly-better strategy — build the project view from the project's provenance file rather than `project_hint`) is logged in [`BACKLOG.md`](BACKLOG.md) for later discussion.
> - **⏳ Still open (non-blocking):** (1) **confirm researcher machines resolve the UNC `\\GJESUS3\gjesus3\gjesus3-data`** (the new default `--link-base`) — else regen with a `J:\gjesus3-data` drive-letter base; (2) remaining *Next* features (saved/shareable searches, CSV/methods export, counts dashboard, per-row XNAT/OMERO launch links). The "select-in-Finder → assemble a project" idea is logged in [`BACKLOG.md`](BACKLOG.md).
>
> ---
> *History below (pre-2026-06-15) — the stale "quasi-production / purge-after-exhibition" framing in some entries is superseded by the true-production state above (the purge already happened 2026-06-10).*

> **▶ MRI HISTORICAL INGEST LANDED (2026-06-13) — 6,405 acqs, "ingest what we have, flag the gaps" iteration.** All **jrc/MFB** ParaVision exams pulled **read-only/copy** from `kenia` (`/opt/PV-7.0.0` + `/opt/PV6.0.1`, 935 sessions / 490 GB → `D:\projects\mri`) and ingested into **`J:\gjesus3-data`** via two new YAML configs (`tools/configs/mri_jrc_{animalfirst,projfirst}.yaml` — two folder orderings; `researcher: "NA"`; `project_hint: ae-biomegune-${project_code}`, NI-aligned). **Decision (user): skip+flag the no-DICOM exams now, convert later** (avoid empty placeholders). **Verified:** of 10,316 exams, **6,405 with DICOMs ingested, 0 failures**; 16 project workspaces (incl. `0525`/`0424` **reused from NI** — cross-modality); 6,405 raw folders + DICOMs + hard-links; registry 132→6,537 rows.
> - **Flagged + held for the conversion pass:** **3,911 no-DICOM exams** moved to `D:\projects\mri\_flagged_no_dicom\` (raw `fid`/`2dseq` intact) + work-list `D:\projects\mri\mri_flagged_no_dicom.csv`. Convert with the designer's [`equipment/mri-platform/mri_no_dicom_regeneration_runbook.md`](../equipment/mri-platform/mri_no_dicom_regeneration_runbook.md), then idempotent re-ingest.
> - **Subject-id back-fill DONE (2026-06-13):** 452 acqs (projects **1521/0619/0618**) ingested with `subject_ids = "<animal>-AE-biomaGUNE-None"` because the **facility DB returns a null project alias for those 3 projects** (animals resolve, only the alias is null — verified by direct lookup; our parse `discovered.project_code` was correct). Recomposed all 452 from `discovered.project_code` → fixed **sidecars + registry** (registry backed up `registry_raw.csv.bak.backfill-none-*`; **0 `-None` remain**). Root cause is a source-DB gap → see BACKLOG (harden ingest to fall back when the DB alias is null + ask data office to fix those 3 project records).
> - **Normal non-blocking:** 165 DB-miss recoveries queued to `pending_subject_metadata.csv` (the designed deferred-recovery path). **Anatomy `is_whole_body` null for all** (per-acq, not batch-settable) → BACKLOG back-fill/auto-derive lane.

> **▶ PRODUCTION HISTORICAL INGEST STARTED (2026-06-12) — first true-production preload landed + verified.** NI **archive-mode** preload of **`\\cicmgsp02\gnuclear2$\2025\Jesus\Irene` = 132 acqs** (66 PET + 66 CT; series 0525 + 1207; animals m1–m28; **all single-animal**, 0 phantom) into the production container **`J:\gjesus3-data`**. Pipeline: `tools/extract_ni_archives.py` (→ `D:\projects\Nuke\gnuclear2_jesus_2025`, since **deleted** post-verify) → `tools/ingest_raw.py --config tools/configs/ni_jesus_archive_2025.yaml --nas-root J:/gjesus3-data` (**new production config**, commit `2cb5033`; copied from `molecubes_ni.yaml` — **no registry/sidecar shape change**). **Verified:** 132/132 rows (0 failed); 132 slim raw DICOM folders (~28 MB ea vs ~6 GB source); 132 project hard-links (`0525`:96 / `0424`:36); **PROJ-0001 `ae-biomegune-0525` + PROJ-0002 `ae-biomegune-0424` auto-created**; `subject_ids` **132/132 populated, 0 pending — every animal resolved LIVE in the animal-facility DB** (species/strain/sex/age in sidecar, `source=animal-facility-db`); checksums present. The empty production registry was born with the new `subject_ids` schema (`append_row` auto-creates — no seeding). The `subject_id`→`subject_ids` rename rode through transparently (single-animal = bare id). **Coverage:** gnuclear2$ holds only Irene/2025 for Jesus; older MFB NI + other researchers are on the live box (blocked, see [`ni_live_sync_handoff.md`](ni_live_sync_handoff.md)) or future gnuclear3. **Still to preload (NOT blocked by NI-live):** MRI (`kenia` SFTP) + microscopy (Axioscan7 `S:\goptical`, Cell/Confocal `K:\gjesus\Ainhize`) — see [`equipment/historical_data_archives.md`](../equipment/historical_data_archives.md).

> **▶ CURRENT (2026-06-12) — correction pass + `origin/main` merge, on branch `fix/correction-pass-2026-06` (pushed; merging now).** The post-first-review correction pass is implemented + sandbox-verified (plan: [`correction_pass_plan.md`](correction_pass_plan.md) + [`correction_pass_handoff.md`](correction_pass_handoff.md)). **Correctness/integrity (gates historical-data ingest):** registry lock + durable ACQ-ID reservation + BOM/trailing-newline-safe CSV appends (new `tools/ingest/{locking,csv_safe}.py`, [06_REGISTRIES §2.7](../mfb-rdm-docs/06_REGISTRIES.md)); archive re-run idempotency; crash-after-copy rollback (registry append = commit point; `--delete-source` moved to the end); empty-folder fail-loud; cross-drive relpath guard. **Operator:** GUI dry-run defaults ON (loud banner + summary); CELL + LSM9 recipes; GUI `.exe` builds + frozen-path bug fixed. **Docs:** `START_HERE.md` / `GLOSSARY.md` / `CHANGELOG.md`; S2 DRAFT→DECIDED; S3 suggested vocabularies; header de-bloat; stale-ref + DB-"blocked-on-IT" fixes.
> **Merge with the designer's true-production-restart work (`origin/main` → `eb95fc6`):** their fresh registry schema (`sample_organism` + `subject_id` + `anatomical_entity`, Auto projections via `build_row(subject=, anatomy=)`) **supersedes the correction-pass S1** — adopted on merge, my single-column S1 dropped; they independently fixed the cross-drive relpath (= item 7); they added the **multi-animal NI solution** (1 scan = 1 entry, 1–4 animals as a list; new top-level `registry_subjects.csv`). **NI-LIVE-08 DECIDED 2026-06-12 (user):** singular `subject_id` → packed **`subject_ids`** (`;`-joined, always-a-list, visible multiplicity) + a **true one-row-per-subject `registry_subjects.csv`** (grab-what-we-can; updatable; static-vs-dynamic deferred) + **minimal** sidecar subset; the data-office deferred-recovery path (`pending_subject_metadata.csv` → `recover_subject_metadata.py`) is the immutability "cheat" (already built); **no mapping/junction table now** (deferred query-layer option); `sample_type`/`sample_id`/GUIs untouched. **The `subject_ids` packing + the subjects-table writer + minimal sidecar = the designer's redesign** around these decisions (this merge only lands the schema consolidation + keeps the correction-pass work). Full record: [`origin_main_merge_review.md`](origin_main_merge_review.md).
> **Awaiting the user (external deps, surfaced not blocking):** (1) a representative `.czi` → verify the frozen GUI's end-to-end metadata read; (2) run `tools/diagnostics/test_oslink.py` on the **MRI (Linux)** + **NI (Mac-fronted)** mounts → choose hard-link vs fallback per instrument; (3) sign-off to run a smoke ingest against the live `J:\gjesus3-data` registry; (4) optional `.my.cnf` install for DB auto-fill ([tools/operator/README.md](../tools/operator/README.md)). **(Push is fixed — this clone now uses the SSH `origin`, so it pushes headless from a remote session.)**

> **▶ NI LIVE-MACHINE INGEST — Step-1 done; Step-2 path forward PROPOSED (2026-06-12, awaiting go-ahead).** Designer's read-only discovery (`tools/ni_live_discover.py`) + the data-office `project-conflict` flag/grammar test are merged + validated (262-acq snapshot: 254/262 DB-keyable, 102 multi-animal). Full Step-2→3 plan: [`ni_live_step2_plan.md`](ni_live_step2_plan.md). **✅ D1 FIXED 2026-06-12 (user-directed, before any historical ingest):** §3B/§3C/§7 + [`06_REGISTRIES §2.3/§2.3.2`](../mfb-rdm-docs/06_REGISTRIES.md) now mandate `registry_subjects.csv` = **one row per subject** (static record), **not** a `(acq_id, animal)` junction table (the vetoed shape); scan→animal link = packed `subject_ids`; per-(scan, animal) position/age-at-acq → sidecar `subjects:[…]`. No such writer exists in code yet (verified — only a comment in `registry.py`), so the spec is corrected before anything could build the wrong table. **✅ Column rename DONE 2026-06-12:** the AUTO column `subject_id` → packed `subject_ids` is applied in code (`registry.py`/`resolver.py`/`build_row`; `test_registry_fields` green), `migrate_registry_columns.py` is now rename-aware, and the sandbox header is migrated (28 cols; the purged production registry is born fresh). **Still open (deferred — live-ingest only):** D2 NI-LIVE-09 + D3 NI-LIVE-07 confirms; the rest of Lane-A (multi-animal list-packing in `build_row` + the one-row-per-subject `registry_subjects.csv` writer) ‖ Lane-B live glue (`molecubes_ni_live.yaml`); Gate-0 `os.link`-on-Mac → vetted sandbox one-shot.

> **▶ PREVIOUS (2026-06-10) — operator hand-off, ✅ MERGED to `main` (branch `operator-ingest-tooling` deleted).** The NI + MRI **command-line tools are DONE and user-verified end-to-end** (real ingests run + accepted). The **microscopy GUI is operator-tested + accepted** — browser click-through on AxioScan against the test NAS; a builder "start from an existing saved recipe" convenience + a Windows-SMB default NAS root (`\\gjesus3\gjesus3\gjesus3-data`) added 2026-06-10. Test plan in [`tools/operator/TESTING.md`](../tools/operator/TESTING.md). The earlier 2026-06-09 session also landed: **the global person/role rename** (registry `operator`→`researcher`; `operator` sidecar-only; permission roles Operator→Tech, User→Researcher; "user"=software only — [06_REGISTRIES §2.3a-bis](../mfb-rdm-docs/06_REGISTRIES.md), live registry header migrated); **`mri-ingest --operator` REQUIRED**; NI PI fix + MRI `pi_initials`/`sample_id` alignment + `pi_group_lookup.yaml`; condition/anatomy CLI flags + prompts; anatomy extended to tissue; condition for cells. Small follow-ups + the GUI "researcher REQUIRED" item are in [`BACKLOG.md`](BACKLOG.md). Detail memory: `[[operator-ingest-tooling]]` + `[[role_terminology]]`.

> **▶ GUI LAUNCH CHECKLIST — ✅ COMPLETE 2026-06-10.** The microscopy GUI saves operator recipes as **YAML to a configurable folder defaulting to `<NAS>/recipes`** (follows the Destination NAS), and the GUI's default Destination NAS is now the Windows-SMB UNC **`\\gjesus3\gjesus3\gjesus3-data`** (was the WSL `/mnt/gjesus3` path). Both pre-launch operational steps are done:
> - [x] **NAS `recipes/` write access granted** (Ryan, 2026-06-10) — operators can create/edit recipes under `<NAS>/recipes` via the existing `CICBIOMAGUNE\GJesus` group, same pattern as `projects\`/`staging\`. (Ties into the §4.3 permission model.)
> - [x] **Existing repo recipe migrated onto the NAS** (2026-06-10) — `axioscan7_7chunk_section` re-saved as YAML at `<NAS>/recipes/axioscan7_7chunk_section.yaml` (round-trip-verified: identical overrides); the `tools/operator/recipes/test.json` throwaway was skipped. The GUI still reads the repo `tools/operator/recipes/` as a read-only *seed* source (data-office machine only).
...
>
> **▶ UP NEXT (deferred to the post-exhibition true-production restart — the purge is its trigger):** Phase 4 **re-ingest of all rounds** through the non-blocking enrichment writer (the purge makes "backfill" moot — the ~487 acqs come back fresh with `subject:`/`condition:`/`anatomy:`, `subject:` auto-filled from the animal DB). ✅ **Registry schema DONE 2026-06-10** — the fresh header added `sample_organism` + `subject_id` + `anatomical_entity` (Auto projections of the enrichment blocks; no migration, registry was empty). **Remaining restart steps:** the `sample_id`-rule template update (organism→subject id / tissue→specimen label), then the re-ingest of all rounds. None blocks handoff or new ingests. **Moved to [`BACKLOG.md`](BACKLOG.md) (2026-06-10, post-launch):** the **NI live-machine import** work (Unai/Irene — live Mac, hard access, different folder layout) and **all independent/second-stage tooling** (`create_publication`, `log_activity`, project-level NIfTI, Excel→study-metadata importer, DICOM full-mode extractor, `--lightweight`). **Right after launch (does NOT block it):** the operator **create-but-not-modify SMB test on `raw\`** — the ACLs visibly hold, but the real check needs a real operator account (Ryan to test with **Irene**); superusers can't self-test (Full masks the restriction). **NB: the DRAFT `sample_id`/`sample_type` conventions are Data Office decisions refined with user input — there is NO PI sign-off gate in this project.**

> **⚠️ TRUE PRODUCTION since the 2026-06-10 restart.** The team exhibition is over and the quasi-prod pilot was purged 2026-06-10; `J:\gjesus3-data` is now the live true-production system being populated round by round (see §0 top). "Done" means done in true production; there is **no pending purge/restart**. (Older entries below were written under the quasi-prod framing — historical.)
>
...
|------|---------|-----------|
| Nuclear Imaging live-machine import (`PET`/`SPECT`/`CT`) | → [`BACKLOG.md`](BACKLOG.md) | **Moved to BACKLOG 2026-06-10 (post-launch).** Saw the live Mac with Unai; access is hard (machine in heavy scan use), the live folder layout differs from the archive `.tgz`, and Irene's early-adopter notes need folding in. Archive mode covers the exhibition. |
| Animal/histology-mode Cell Observer | §4.6.B Deferred | Real example folder of historical histology work; backfill round. |
| Operator create-but-not-modify test on `raw\` | §4.3, §6 | **Right after launch (does NOT block it).** Ryan to test with **Irene** (a real operator account) on-site — the ACLs visibly hold, but superusers can't self-test (Full masks the restriction). Permissions APPLIED 2026-06-02; only the SMB-translation check remains. |
...
- [ ] → **[BACKLOG](BACKLOG.md)** (independent tooling, 2026-06-10): Implement `--lightweight` flag in `ingest_raw.py` — tested in Section 4.2
- [ ] → **[BACKLOG](BACKLOG.md)** (independent tooling, 2026-06-10): Add NIfTI handling (single file, no archive) — tested in Section 4.8 if applicable
- [ ] → **[BACKLOG](BACKLOG.md)** (independent tooling, 2026-06-10): Implement `backfill_metadata` utility for upgrading lightweight ingests
...
- [ ] **ParaVision → DICOM regeneration for no-DICOM internal MRI acquisitions** (UPDATED 2026-05-29 — Dicomifier identified as candidate; previously assessed as 2-4 week build-from-scratch). When researchers don't run Bruker's GUI DICOM exporter, the acquisition's `pdata/<idx>/dicom/` is missing entirely; round-6 v2 detected 3 of 7 source projects in this state. Currently the ingest registers these as empty-`.data/` placeholders.
  ...
  **Phase 2 status (2026-06-01):**
  ...
  - [x] **Operator documentation + env tooling — DONE 2026-06-12.** Official runbook [`equipment/mri-platform/mri_no_dicom_regeneration_runbook.md`](../equipment/mri-platform/mri_no_dicom_regeneration_runbook.md) (setup → config flag → run from WSL → verify) for the historical-pull team; committed conda env spec [`tools/dicomifier-pilot.environment.yml`](../tools/dicomifier-pilot.environment.yml) (`conda env create -f …`); `tools/validate_dicomifier_pixelspacing.py` now takes a `<ACQ-ID>.data/` path arg (operator post-ingest check: PASS = no `matches_swapped`) instead of hardcoded dev paths. Cross-linked from `equipment/INDEX.md`, `INGEST_CLI.md`, and the MRI workflow notes. Memory: [[mri-no-dicom-regeneration]].
...
- [x] ~~**Anatomy extended to ex-vivo `tissue` (region-only) + standards check**~~ ✅ DONE 2026-06-09 — the `anatomy:` block now applies to `sample_type = tissue` (AxioScan sections): the UBERON `region` the section was cut from, `is_whole_body` **N/A** (stays null, not warned — gate-aware in `tools/ingest/enrichment.py`). **Same UBERON `region` field as in-vivo scan coverage** → one query ("all heart material") spans a cardiac MRI **and** a heart section. Active `anatomy:` block added to `axioscan7.yaml` (region set per batch / per-acq override; **no GUI change** — no `is_whole_body` to toggle for tissue, per Ryan's call). **Standards-checked** (OME / OMERO / REMBI; UBERON / EBI BioSamples): the organ is the biosample's **anatomical entity / "organism part"** (UBERON) — NOT "organ" (too narrow) and NOT REMBI's *spatial* "Location within biosample"; OME/OMERO have no native organ field (map-annotations). **Fixed the `06_REGISTRIES §2.3` miscitation** + reframed `anatomical_entity` as the registry projection of `anatomy.region`. **Cells DEFERRED** (a cell line's source organ is line-static — capture in cell-line/project metadata). Verified gate matrix (tissue→region, no is_whole_body warn; organism unchanged; cells→no anatomy). Docs: `08_METADATA §4.6`, `06_REGISTRIES §2.3/§2.4`, `09_MODALITIES`. **Follow-up (optional):** let the GUI operator set the tissue `region` (today it's config/per-acq only) — e.g. a region field mapped from the `sample_short` organ letter via a lab letter→UBERON table.
- [x] ~~**NI PI correctness fix + MRI/NI identity alignment (from operator-run review)**~~ ✅ DONE 2026-06-09 — **(1) NI PI:** the Molecubes `protocol.txt` "Principal Investigator" holds the *operator's username* (platform-labelling bug, under investigation), so `ni.study.principal_investigator` + `discovered.ni_pi` are now **emptied** (`ni_metadata.py`); the raw value stays untouched in `ni._raw_metadata.protocol_txt`. PHASE-OUT once the platform is fixed. Verified: curated PI `""`, raw `irene` preserved, `discovered.user="irene"` (correct). **(2) MRI `pi_initials`:** the study-folder regex now captures `discovered.pi_initials` ("jrc"), handling the `jrc`/`jrc_` no-separator quirk. **(3) MRI↔NI `sample_id` alignment:** MRI `sample_id` is now `m<animal>_<project>` ("m17_0424") to match NI ("m13_0525"); `session_id` keeps `jrc_id`; canonical subject id stays the DB `subject.facility_animal_id` (already aligned). **(4) PI↔group lookup:** new [`tools/reference/pi_group_lookup.yaml`](../tools/reference/pi_group_lookup.yaml) links PI first name (NI "Jesus") ↔ PI initials (MRI "jrc") ↔ group initials (microscopy "MFB"). Verified live (MRI: pi_initials + sample_id; NI: PI emptied + raw kept). Docs: `09_MODALITIES` (ni_pi row), `06_REGISTRIES §2.3`, `equipment/{mri,nuclear-imaging}` workflow notes, `00_INDEX`. **Deferred to [`BACKLOG.md`](BACKLOG.md):** `--pi`/`--user` operator entry (no clean home until the REG-01 person-schema), NI stage-2 PI-dir batch, stage-3 group defaults + `--group`, wiring the lookup into the tools, curated `mri.subject.id` normalization. **Confirmed not-a-bug:** the `is_whole_body` "asked twice" was the typo re-ask (`flase`→re-prompt→`false`), working as designed.
- [x] ~~**Person/role naming fixed globally**~~ ✅ DONE 2026-06-09 — registry `operator` column → **`researcher`** (who set up the experiment); **`operator`** (who ran the equipment) is now **sidecar-only** (`metadata.json.user_supplied.operator`); permission roles renamed **Operator→Tech, User→Researcher**; **"user"** reserved for software users. Model in [06_REGISTRIES §2.3a-bis](../mfb-rdm-docs/06_REGISTRIES.md). Code (REGISTRY_FIELDS/resolver/build_row/metadata_sidecar/config `_apply_operator`/config_builder), `mri-ingest --operator`, GUI Researcher field, all 6 templates + 10 configs; **live `registry_raw.csv` header migrated** (operator→researcher, 487 rows, `.bak` on NAS; row VALUES wrong-until-purge). Permissions = docs-terminology only (applied ACLs unchanged) + the MRI/NI platform-login note. Verified live end-to-end. Commit `02d4e1d` (code) + docs. Residual prose sweep + people-roster reclassification on [`BACKLOG.md`](BACKLOG.md).
...
- [ ] → **[BACKLOG](BACKLOG.md)** (independent tooling, 2026-06-10): `create_publication` — requirements defined, not yet implemented
- [ ] → **[BACKLOG](BACKLOG.md)** (independent tooling, 2026-06-10): `log_activity` (provenance helper) — requirements defined, not yet implemented
...
- [ ] **Study-metadata work stream** (architecture in [08_METADATA §1](../mfb-rdm-docs/08_METADATA.md), 2026-05-12). Order is roughly: gather → import → close-out. None blocks the first user-driven Phase B ingest.
  ...
  - [ ] → **[BACKLOG](BACKLOG.md)** (independent tooling, 2026-06-10): **Project-level NIfTI generation tool** (NEW 2026-05-20, MRI-driven but generalizable). Reads chosen ACQ-IDs via the project's `raw_linked/` shortcuts, runs `dcm2niix` (or `bruker2nifti`) per acquisition, writes `<ACQ-ID>.nii.gz` under `/projects/<proj>/derived_nifti/`. Removed at project close-out — regenerable from raw if needed later. Aligns with the [13_GJESUS3_ROLE](../mfb-rdm-docs/13_GJESUS3_ROLE.md) reframe (research-facing derivatives belong in projects, not in `/raw/`). Library choice (dcm2niix subprocess vs bruker2nifti Python) TBD when implementation starts. Cross-referenced in [08_METADATA §1.5a](../mfb-rdm-docs/08_METADATA.md).
  - [ ] **Preclinical metadata auto-population — `subject:` + `condition:` blocks** (NEW 2026-05-29, expanded 2026-05-29). Spec landed in [08_METADATA §4.4](../mfb-rdm-docs/08_METADATA.md) (`subject:`) + [08_METADATA §4.5](../mfb-rdm-docs/08_METADATA.md) (`condition:`). Required for `sample_type ∈ {organism, tissue}` — always for internal MRI + NI, typically for animal-derived microscopy. The two blocks have different source pipelines but share a writer.
    ...
    **4-phase work:**
    ...
    - [ ] **Identity model — ingest-tool parse + registry columns** (NEW 2026-06-03; Subject/Sample model [06_REGISTRIES §2.3](../mfb-rdm-docs/06_REGISTRIES.md), **Option B**). Two parts:
        ...
        - [x] ~~**Registry `subject_id` + `anatomical_entity` (+ `sample_organism`) columns**~~ ✅ DONE 2026-06-10 (true-prod restart) — added to the fresh registry header as **Auto projections** of the enrichment blocks (`subject.facility_animal_id`→`subject_id`, `subject.species`→`sample_organism`, `anatomy.region.label`→`anatomical_entity`); `registry.build_row` populates them from the Step 8.4 blocks (enrichment runs before the row is written). **No migration** (registry was empty post-purge); REG-07's `sample_organism`/`anatomical_entity` split folded in as one change. 28-col header. Code: `registry.py` (`REGISTRY_FIELDS`+`build_row`), `resolver.py` (`AUTO_COLUMNS`), `ingest_raw.py` Step 10; docs `06_REGISTRIES §2` / `08_METADATA` / `10_TOOLS` / `00_INDEX`.
