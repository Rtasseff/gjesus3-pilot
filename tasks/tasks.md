...
**Last Updated:** 2026-06-03 (NEW `anatomy:` block — `is_whole_body` + UBERON `region`, `08_METADATA §4.6`; animal-facility DB explored Phase 1; Subject/Sample identity model Option B; `animal_db.py` Phase 2 fetcher done; procedures = structured DB vocab, META-07 retired; §3.2 + `06_REGISTRIES §2.3` + `08_METADATA §4.4/§4.6`)
...
|---|---|---|
| **Preclinical subject metadata DRAFT spec** (species/strain/sex/DOB→age — ARRIVE-aligned) | §3.2 (4-phase animal-DB integration) | ✅ Spec landed + extended. **Phase 1 DONE 2026-06-03** — `animal_facility` MariaDB explored read-only; field mapping + join key confirmed; species/sex normalization; **procedures = structured DB vocab (META-07 retired)**; deferred-recovery §4.4.6. **Subject/Sample identity model adopted (Option B, DRAFT, refines REG-01 — `06_REGISTRIES §2.3`):** `facility_animal_id` = reused canonical `<animal_code>-AE-biomaGUNE-<NNNN>` subject id; registry `subject_id`/`anatomical_entity` columns deferred to true-prod restart. **Phase 2 fetcher `tools/animal_db.py` DONE 2026-06-03** (verified live, self-test green). **Next: Phase 3** — wire into ingest (`subject_from_db:` + sidecar writer). |
...
| **Anatomical coverage DRAFT spec** (`anatomy:` block — `is_whole_body` DECIDED + UBERON `region`) | §3.2 (same Phase 3 writer) | ✅ Spec landed 2026-06-03 (`08_METADATA §4.6` + §4.3 table + `09_MODALITIES` cross-modality note + MRI/NI rows + `00_INDEX`). Dead-simple full-body-vs-ROI boolean for `organism` scans; UBERON ontology (verified starter ids); operator-entered (NOT auto-derivable — DB has no field, DICOM `BodyPartExamined` empty). Phase 3 writer adds the `is_whole_body` REQUIRED-check. META-09. |
  ...
  - [ ] **Preclinical metadata auto-population — `subject:` + `condition:` blocks** (NEW 2026-05-29, expanded 2026-05-29). Spec landed in [08_METADATA §4.4](../mfb-rdm-docs/08_METADATA.md) (`subject:`) + [08_METADATA §4.5](../mfb-rdm-docs/08_METADATA.md) (`condition:`). Required for `sample_type ∈ {organism, tissue}` — always for internal MRI + NI, typically for animal-derived microscopy. The two blocks have different source pipelines but share a writer.
    ...
    **4-phase work:**
    ...
    - [ ] **Phase 3 — Wire into ingest** (covers THREE blocks: `subject:` + `condition:` + `anatomy:`). New YAML field `auto_discover.subject_from_db: true` (default false) for `subject:` DB-fetch. New top-level `condition:` + `anatomy:` YAML blocks (peers to `registry:`) for operator-entered data — resolver-evaluated, same `${discovered.<x>}` interpolation as `registry:`. `metadata_sidecar.py` writes all three:
       ...
       - For `anatomy:` (only `sample_type = organism`): read from YAML `anatomy:` block. **`is_whole_body` REQUIRED-CHECK: writer raises if missing for `organism`** (DECIDED, [08_METADATA §4.6](../mfb-rdm-docs/08_METADATA.md)). `region` (UBERON `{label,ontology,id}`) missing while `is_whole_body=false` → WARN but continue (DRAFT). Optional non-authoritative `auto_hint` may be pre-filled from MRI `ProtocolName`+`geometry.fov` / NI `Scan bed position from..to` (a small future extractor — NOT authoritative). META-09.
       - Per-acquisition override (mixed batches): `/projects/<proj>/metadata/<acq_id>.json` `condition:`/`anatomy:` override YAML batch-level.
