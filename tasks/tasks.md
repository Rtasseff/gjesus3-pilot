...
- [ ] **Operator-facing ingest tools** (design LOCKED 2026-06-03; build DEFERRED, now unblocked) — Windows GUI for microscopy (hybrid recipes+builder, local Flask app packaged as a PyInstaller `.exe`) + dead-simple `ni-ingest`/`mri-ingest` Linux scripts, all over a shared cross-platform `tools/operator/` core that reuses the validated pipeline (no reimplementation). NI-first sequencing. Full plan persisted at [`tasks/operator_ingest_tooling_plan.md`](operator_ingest_tooling_plan.md). The metadata lane it waited on (Phase 3) has landed → ready to build. (This is the work that was previously only in a `.claude/plans/` artifact and never committed — recovered 2026-06-03.)
