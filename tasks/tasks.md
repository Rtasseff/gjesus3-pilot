...
**Last Updated:** 2026-06-18 — researcher Finder MVP built, pushed + global index published, then **put ON HOLD** pending a value/complexity evaluation; focus returning to the NI live-machine sync ingest (branch `feat/finder-mvp`, see Section 0). Full dated history in [CHANGELOG.md](../CHANGELOG.md).
...
> - **⏳ Awaiting the user (3 open decisions):** (1) **confirm researcher machines map the NAS as `J:`** (Ryan's does) — if they use a different letter, or you want UNC portability, regen with the right `--link-base`; (2) **publish the per-project indexes too?** (`--per-project` writes an `index.html` into every project folder — built + tested; held off because each exposes that project's acqs to anyone who can read the folder); (3) **evaluate MVP value/complexity**, then decide whether to **merge `feat/finder-mvp` → main + push** and which *Next* features (richer detail panel, saved/shareable searches, CSV/methods-section export, counts dashboard, per-row XNAT/OMERO launch links). Product framing + scope/limits in [`tools/FINDER.md`](../tools/FINDER.md).
