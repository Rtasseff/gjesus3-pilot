...
- **SegBioMed segmentation harvest — answered 2026-08-21; two ingests proposed, one BLOCKED.** The SegBioMed project deposited ~370 MB / ~1,050 files of segmentation ground truth into `curated_datasets\_incoming\seg-harvest-2026-08-20\` (pre-promotion; sources read-only; sha256 manifest) and asked for four things. **Nothing was ingested, promoted, deployed or deleted** — the asks need decisions, not code. Reply appended to `projects\Imaging\SegBioMed\harvest\MEMO_for_gjesus3_agent.md`; findings filed in [`BACKLOG.md`](BACKLOG.md); narrative in [`../CHANGELOG.md`](../CHANGELOG.md) 2026-08-21.
  - **Corrected their model of the archive:** the registry reaches back to **2018-04-25**, and both protocols they treated as absent are present — `Proyecto 1019` = **`PROJ-0006`** (461 MRI acqs, 2022, 33 animals, `closed`) and their "unmatched" `1123_205…304` cohort = **`PROJ-0014`**, where animals **205–301 already hold 187 AxioScan histology acquisitions**.
  - **⚠️ BLOCKED — the 2026 NI lung study** (`S:\gnuclear\2026\Jesus\Itziar\1123`): **44 of 233 animal folders contradict their own contents** about which animal they are — a constant **`+21` offset** across the 40 baseline folders (resolved *to the folders*, on three independent lines of evidence), plus a transposed pair, a mixed-timepoint folder, and an `Ex vivo` folder holding three animals. **Neither folder-derived nor filename-derived subject ids are safe here**; 4 conflicts need the researcher or the facility DB. Same failure class as the PROJ-0056 `rN` misattribution. Also: the study is **NIfTI, not DICOM**, which is why the 2026-08-12 historical pull could not see it (a structural blind spot, not a missed folder), and **no instrument code exists for optical imaging** although two of its sessions are DiR fluorescence.
  - **Ready to plan — historical MRI ingest of `Proyecto 1019`:** `K:\gjesus\MRI\Proyecto 1019` holds 95 ParaVision study folders, **9 already in the registry** (so the tree is a proven source), leaving **86 folders / 56 sessions from 2021 — 40,811 files, 37.3 GB**. It is what would let their 135 cardiac masks satisfy the promotion checklist (today **0 of 135** trace to an ACQ-ID). Needs a go-ahead, and a decision on reopening `closed` PROJ-0006.
  - **Awaiting Ryan/PI — CDS-01:** deploying `curated_datasets/` is an open ❓ EVALUATING scope decision, so it was left alone. Their `DS-SEG-0002` **passes** §6.2 traceability (**821 of 821** ACQ-IDs resolve, 12 projects); `DS-SEG-0001` fails it pending the 1019 ingest.
