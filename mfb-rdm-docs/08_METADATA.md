...
| Registry `subject_id` column | ✅ ADDED 2026-06-11 (S1) — auto-populated from the sidecar `subject.facility_animal_id` ([06_REGISTRIES §2.2](06_REGISTRIES.md)); empty for non-animal samples. The `anatomical_entity` column stays 🔶 DEFERRED to the true-production restart (sidecar carries it meanwhile). |
...
| META-08 | Subject/Sample two-tier identity model (reused facility animal ID as subject; registry `subject_id`/`anatomical_entity` columns) — confirm at PI sign-off (REG-01) | Data Mgmt Lead + PI | 🔶 DRAFT 2026-06-03 — model adopted (Option B), grounded in FAIR/ISA/REMBI/BIDS/XNAT; see [06_REGISTRIES §2.3](06_REGISTRIES.md). Registry **`subject_id` column added 2026-06-11 (S1)**; `anatomical_entity` column still deferred to true-prod restart. |
