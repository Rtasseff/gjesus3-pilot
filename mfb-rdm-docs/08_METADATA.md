...
|----------|------------------|--------|------|----------------------|
| `registry_raw.csv` | Indexed core fields (`acq_id`, `instrument`, `sample_id`, `sample_type`, `project_id`, etc.). See [06_REGISTRIES](06_REGISTRIES.md). | Auto + Operator (via YAML `registry:` block) | At ingest | Admin-only (corrections) |
...
| `session_extra` (when supplied) | **NI live only, 2026-06-29.** Free-form `key: value` operator-supplied per-SESSION metadata that isn't anywhere in the acquisition files — chiefly the **tracer / compound** (study-level knowledge). Entered via the `extra_metadata` column (`key=value;key=value`) of the NI corrections worksheet (`ni-ingest --live --plan/--corrections`; [10_TOOLS](10_TOOLS.md), `tools/ingest/ni_corrections.py`) and stashed by `apply_post` onto every acquisition of that session. **Only present when the operator supplied it** — every other sidecar is byte-for-byte unchanged. Aligns with the NI backlog "tracer compound NOT in the data — must come from the researchers/study records." |
...
> **Note — multi-animal acquisitions: the `subjects:[]` array.** The block above is the **single** `subject:` key (one animal). A Molecubes NI scan can image **1–4 animals at once** (NI-LIVE-08); for those the sidecar *also* carries a top-level **`subjects:` array** — one full block (same schema as above) per animal — written by `metadata_sidecar.build_sidecar(..., subjects=[...])` **alongside** the scalar `subject:` key, which keeps the first/primary animal for back-compat. The registry projects the whole set into the packed, `;`-joined `subject_ids` column. Single-animal acquisitions carry only the scalar `subject:` (no `subjects:` array). The animals are parsed by the `auto_discover.subject_parse` block ([10_TOOLS §2.1.6](10_TOOLS.md)).
...
1. **Animal facility DB** — authoritative. **Explored 2026-06-02** (MariaDB `animal_facility` schema; read-only via `~/.my.cnf`). Lookup = `projects.projectAlias = <NNNN>` (from the project name `AE-biomaGUNE-NNNN`) + `animals.animal_code` (the leading number of the instrument short code, `m13`→`13`); returns species/strain/sex/DOB + the structured `procedures` list. Set `source: "animal-facility-db"`. When the DB lookup at ingest fails (animal not yet in the DB, or no credentials on the operator's machine), fall through to the deferred-recovery path (§4.4.6) — write `source: "pending-db"` and queue for superuser recovery, rather than dropping straight to the lower-confidence sources below.
...
| **Recommended for** (non-blocking) | `sample_type ∈ {organism, tissue}` | `sample_type ∈ {organism, tissue, cells}` |
...
| Microscopy (tissue) anatomy **auto-derive** (sample-id organ code → region) | ✅ IMPLEMENTED 2026-06-14 — for ex-vivo `tissue` the organ is in the `.czi` sample-id suffix (`ID12Lu`→lung, `ID29H`→heart). `anatomy_derive.derive_microscopy_anatomy` maps it via the **operator-keyed** reference map `tools/reference/microscopy_organ_map.yaml` (the suffix vocabulary differs by operator — AUA `Lu/Li/K`, MBC `H/B/L/HL`), wired into `enrichment._build_anatomy` (tissue path; operator-set wins; high-confidence only — unmapped/ambiguous codes like AUA `T`=tumor stay null). `is_whole_body` stays null (N/A for a section). UBERON ids OLS-verified. |
| Back-fill of existing microscopy acqs | ✅ TOOL IMPLEMENTED 2026-06-14 — `tools/backfill_microscopy_anatomy.py` applies the SAME organ map to already-ingested microscopy sidecars + the registry `anatomical_entity` column (dry-run default; atomic + verify; only unset acqs). Operator runbook: [`tools/ANATOMY_BACKFILL.md`](../tools/ANATOMY_BACKFILL.md). |
