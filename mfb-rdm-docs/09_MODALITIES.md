...
>
> **`subject:` block** — *who is this animal?* Required fields: `species` / `strain` / `sex` / `date_of_birth` → derived `age_at_acquisition` (the ARRIVE-aligned required fields, DECIDED; DOB added 2026-06-02, age computed from it). Optional `procedures` free-text. Per-subject, fixed. Sourced from the **animal-facility DB** (access obtained 2026-06-02); ingest-time DB miss / no-credentials WARNs and queues for superuser recovery rather than failing (§4.4.6). Full schema in [08_METADATA §4.4](08_METADATA.md).
>
