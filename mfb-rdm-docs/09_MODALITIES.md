...
>
> **`anatomy:` block** (in-vivo scans only) — *full body or region of interest?* Highly recommended (**non-blocking**, §4.7): `is_whole_body` (**tri-state** `true`/`false`/`null=unknown` — the dead-simple flag, WARN if null) + UBERON-coded `region` when not whole-body. Operator-entered (not in the animal-DB; DICOM `BodyPartExamined` is empty upstream); optional non-authoritative auto-hint from MRI ProtocolName+FOV / NI bed-range. Full schema in [08_METADATA §4.6](08_METADATA.md).
>
