...
>
> **`condition:` block** — *what study state is this acquisition?* Required: `is_control` (**DECIDED, strict boolean** — the enforceable healthy-vs-case flag); DRAFT-required free-text: `disease_model` / `disease_state`. Per-acquisition, varies (same animal can be baseline + post-MI). Full schema in [08_METADATA §4.5](08_METADATA.md).
>
> **`anatomy:` block** (in-vivo scans only) — *full body or region of interest?* Required: `is_whole_body` (**DECIDED, strict boolean** — the dead-simple full-body-vs-ROI flag) + UBERON-coded `region` (DRAFT-required when not whole-body). Operator-entered (not in the animal-DB; DICOM `BodyPartExamined` is empty upstream); optional non-authoritative auto-hint from MRI ProtocolName+FOV / NI bed-range. Full schema in [08_METADATA §4.6](08_METADATA.md).
>
> **Applicability per instrument in current scope:**
> - Internal MRI (§1.4) and Nuclear Imaging (§1.5) — `subject:` + `condition:` + **`anatomy:`** always required (sample is always an organism; in-vivo scans need anatomical coverage).
...
| **Condition / disease-state metadata** (DRAFT 2026-05-29) | ✅ **Required** — `sample_type = organism`. Sidecar `condition:` block must carry `is_control` (DECIDED boolean) + DRAFT-required `disease_model` + `disease_state`. Particularly important for MRI workloads which mix cardiac MI / Alzheimer / oncology / metabolic models in the same cohort. Operator-entered (no auto-source — disease state is study-design, not animal property). See [08_METADATA §4.5](08_METADATA.md). |
| **Anatomical coverage** (DRAFT 2026-06-03) | ✅ **Required** — `sample_type = organism`. Sidecar `anatomy:` block must carry `is_whole_body` (DECIDED boolean — full-body vs ROI) + UBERON-coded `region` when not whole-body. ParaVision exposes only weak hints (`ProtocolName` e.g. `1_Localizer_multi_slice` + `geometry.fov`); `BodyPartExamined` is empty. Operator-entered with optional non-authoritative auto-hint. See [08_METADATA §4.6](08_METADATA.md). |
...
| **Condition / disease-state metadata** (DRAFT 2026-05-29) | ✅ **Required** — `sample_type = organism`. Sidecar `condition:` block: `is_control` (DECIDED boolean) + DRAFT-required `disease_model` + `disease_state` + optional `treatment` (e.g. injected tracer + dose for PET). Operator-entered. The `protocol.txt` `Scan protocol` + `Isotope` + `Activity` fields are NOT a substitute (they describe acquisition, not subject disease state). See [08_METADATA §4.5](08_METADATA.md). |
| **Anatomical coverage** (DRAFT 2026-06-03) | ✅ **Required** — `sample_type = organism`. Sidecar `anatomy:` block: `is_whole_body` (DECIDED boolean — full-body vs ROI) + UBERON-coded `region` when not whole-body. Molecubes hint: the `protocol.txt` `Scan bed position from X to Y` range correlates with axial coverage (more positions ≈ whole-body); `BodyPartExamined` is empty; `SeriesDescription` carries only animal+recon. Operator-entered with optional non-authoritative auto-hint. See [08_METADATA §4.6](08_METADATA.md). |
