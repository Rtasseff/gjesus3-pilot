...
**Status:** ✅ **TRUE PRODUCTION** (active since the 2026-06-10 restart). Live scale: ~13,555 acquisitions in `/raw/`, ~50 projects (each hard-linked to its raw data), **1,124** subjects in `registry_subjects.csv`; `publications/` deferred (empty).

**Last Updated:** 2026-09-04 — **the operator GUI's MRI page can now name the destination project from each scan's own metadata.** The "project name" field was a plain text box, so it held one static string per run and an operator pulling several protocols at once had to split the run or hand-edit YAML; it is now a token field with its own (deliberately narrower) `${discovered.*}` palette, resolved per scan. **No engine or schema change** — `registry.project_name` has always been token-resolvable and the `mri_bruker` template's own default is an expression ([10_TOOLS §5.2](10_TOOLS.md)). Preview now lists every project a run would touch, with scan counts and a flag on the ones it would create. Prior: 2026-08-16 — **the 444 `-AE-biomaGUNE-None` subject identifiers are repaired in true production, and the composer that produced them is fixed.** Four animal protocols (`0219` / `0618` / `0619` / `1521`) carry a NULL `projectAlias` in the facility DB, so `animal_db` composed the literal `<n>-AE-biomaGUNE-None` — **ambiguous**, because all four collapse onto one id and the subjects-table upsert then merged two different animals into one row (3 collided ids, 47 acquisition rows, wrong sex/strain/DOB for one of each pair). The biology was never wrong; only the composed label. `compose_subject_id` now **refuses** a null alias and `_query_subject` composes from the caller's, with `ingest/enrichment.py` degrading to a blank id so the non-blocking contract ([08_METADATA 4.7](08_METADATA.md)) holds; `validate_registries` reports one as an **ERROR** ([10_TOOLS 3.2](10_TOOLS.md)). Repair run live: 444 sidecars + 444 registry rows, `registry_subjects.csv` 1,146 → **1,124**, `validate_registries` **0 errors / 0 warnings** across all 15,474 rows. Two live byte-fidelity hazards fixed en route (a `utf-8-sig` round-trip would have added a BOM to the no-BOM registry; `_write_sidecar` flipped LF→CRLF on a NOT-uniform archive — 314 LF / 130 CRLF — and now **preserves** each file's terminator). Root cause `ingest/metadata_sidecar.py` filed in [tasks/BACKLOG.md](../tasks/BACKLOG.md). Prior: 2026-08-13 — **DTS24 collaborator cardiac MRI LIVE in production** — 75 acquisitions in `PROJ-0054`, ingested with two new metadata capabilities: new sidecar block `user_provided_metadata` for operator-supplied tables ([08_METADATA §4.9](08_METADATA.md), [10_TOOLS §2.1.7](10_TOOLS.md)) and opt-in curated DICOM-header extraction with a human-subject privacy allow-list ([08_METADATA §4.10](08_METADATA.md), [10_TOOLS §2.1.8](10_TOOLS.md)); backlog META-10/11/12 raised (ISA study level, clinical measurements as their own data type, human-data policy). A **silent wrong-date bug** was found and fixed mid-run: `acquisition_datetime: NA` delegates to `dicom_utils.extract_study_date`, which reads only the first 20 DICOM instances and then falls back to **today's date** as a WARN — so a nested source can bake a wrong date into the ACQ-ID, the `/raw/` path and the link name while the batch still reports success. Not DTS24-specific; see [tasks/BACKLOG.md](../tasks/BACKLOG.md). Prior: 2026-08-12 (later same day) — **project folders are researcher-owned** (new [05_PROJECTS §3a](05_PROJECTS.md), ✅ DECIDED): the system creates, populates, documents and teaches, but mandates nothing; researchers may reorganise or delete anything inside their project folder, **including hard links**, so **no system-of-record fact may be derived from a project folder's contents** — `/raw/` + `registries/` are the record and are self-sufficient without `/projects/`. With it, **`registry_raw.project_id` is write-once and names exactly ONE project** ([06_REGISTRIES §2.3b](06_REGISTRIES.md), mirror `tools/ingest/project_ids.py`) — **superseding the semicolon-list decision of the day before**: sharing an acquisition across projects is supported at the filesystem level (link + the destination project's provenance) but is deliberately **not registered**, and the genuine many-to-many is backlogged against the future metadata database. The multi-value *readers* were kept; only the writer policy changed. Same day, earlier — **Project Manager GUI** built and ✅ deployed (researcher-facing: update / create a project, add `/raw/` acquisitions as hard links, copy local files in) — new [10_TOOLS §5.3](10_TOOLS.md). With it: ~~`project_id` becomes a semicolon-separated list~~ (**superseded same day**, above); the **recommended project subfolders** `raw_linked/` · `working/` · `outputs/` · `metadata/` are created by every tool and backfilled ([05_PROJECTS §3](05_PROJECTS.md) — the 🕗 on `metadata/` is *narrowed* to its contents, not the directory); and **anyone with access may create a project, but only through the system** ([05_PROJECTS §10](05_PROJECTS.md), [RESEARCHER_GUIDE §4](../RESEARCHER_GUIDE.md)). `registry_projects.csv` gained a locked/atomic writer ([06_REGISTRIES §4.4](06_REGISTRIES.md)). Prior: 2026-08-02 — **project reference model** ✅ DECIDED: the "project hint" concept is retired; a project has a `project_id` (`PROJ-XXXX`, machine key, the `registry_raw` column) and a `name` (human key — **and its folder name, verbatim**). New owning section [05_PROJECTS §2a](05_PROJECTS.md); mirrors updated in [06_REGISTRIES](06_REGISTRIES.md) + [10_TOOLS](10_TOOLS.md); forward rule ("one concept → one name → one owning section → one construction site") in [CONTRIBUTING-docs](../CONTRIBUTING-docs.md). Prior: 2026-07-24 — scheduled global Finder rebuild handed to the separate **WorkstationOps** app (`finder-refresh` op); this repo keeps only the generator. Boundary, health check, and the repo-move interdependency are in [11_OPERATIONS §5.6](11_OPERATIONS.md). Recent index-level milestones are tabled under [Version History](#version-history) below; the full dated history lives in [CHANGELOG.md](../CHANGELOG.md).
...
**Contributor & agent rules → [CLAUDE.md](../CLAUDE.md) + [CONTRIBUTING-docs.md](../CONTRIBUTING-docs.md)** (doc-architecture, status markers, cross-reference mirror rules, style).

> ### System Lifecycle
> gjesus3 has been in **true production since the 2026-06-10 restart**. The earlier quasi-production pilot — each instrument iterated test → purge → accept, then a whole-system purge after the team exhibition — is **complete and historical**: that purge **already happened on 2026-06-10**. There is **no future exhibition, purge, or restart pending**, and all data is now **real and retained long-term**. "Done" in this project means "done in true production."
...
**Legend:** ✅ DECIDED / Current | 🔶 DRAFT | ⚠️ GAP | ❓ EVALUATING | 📋 INPUT-NEEDED | 🕗 PLANNED / DEFERRED
...
### Confirmed Equipment (in production)
...
|----------|-------------------|-------|
| Electron microscopy (SEM/TEM) | EM platform | Mainly nanomaterial characterization; need confirmation of use case fit. The `EM` raw ecosystem is **reserved but not deployed** — no EM data is ingested today. |
...
| ✅ Registry schema (DECIDED + LIVE since the 2026-06-10 restart) | `registry_raw.csv` = **28 columns, finalized** (fresh-header layout incl. the Auto columns `sample_organism`, packed `subject_ids`, `anatomical_entity`); sibling tables `registry_projects.csv`, `registry_subjects.csv` (one row per subject), `registry_publications.csv` (🕗 PLANNED/empty) | The restart applied the fresh header; the column set is stable and enforced by the defensive header check in `registry.append_row`. `subject_ids` (`;`-joined, multi-animal-aware; renamed from `subject_id` 2026-06-12) is auto-populated. See [06_REGISTRIES §2.2](06_REGISTRIES.md). |
| Raw structure | **Ecosystem → Year → Month → Acquisition** | Organized by data ecosystem (MICROSCOPY, DICOM; EM *reserved, not deployed*) — stable, maps to tooling, avoids folder proliferation from new instruments or hybrids |
...
| ✅ Project link method (DECIDED + APPLIED 2026-06-02) | **NTFS/SMB hard links** (since 2026-06-02; the older Windows `.lnk` shortcut method is **RETIRED** — 283 links migrated) | Project copy is a real file identical to raw — same inode, zero extra storage, shares raw's ACL (read-only carries through). Folder-primary `.data` → real folder of per-file hard links. Drives adoption (looks like a normal file). See [10_TOOLS §2.1.1](10_TOOLS.md#211-project-linking--hard-links-current-over-lnk-shortcuts) |
...
| ✅ **Metadata-location split** (DECIDED 2026-05-12) | Acquisition-level metadata (auto + Operator at ingest) lives in `/raw/<ACQ-ID>/metadata.json` (immutable) — **LIVE.** Study-level metadata (researcher-supplied: aim, biosample context, experimental groups) is designed to live in `/projects/<proj>/metadata/` (mutable by project owners). | Aligns with REMBI's Study/Biosample/Acquisition hierarchy; keeps `/raw/` strictly read-only post-deposit without blocking researcher metadata work; isolates the ephemeral working-space writes from the permanent archive. Requires project close-out to preserve study metadata into `/raw/` before project deletion. **Acquisition-level sidecars are deployed; the study-level `/projects/<proj>/metadata/` half is 🕗 PLANNED (Phase 4) — present on 0 of the ~50 live projects today (see [tasks/BACKLOG.md](../tasks/BACKLOG.md)).** See [08_METADATA §1](08_METADATA.md), [05_PROJECTS §4.x](05_PROJECTS.md), [11_OPERATIONS §2.1](11_OPERATIONS.md). |
| ✅ **Role terminology + User role** (DECIDED 2026-05-12; terminology finalized globally 2026-06-09) | Fourth role added alongside Owner / Data Mgmt Lead / Operator. Researchers are Users — read-only on `/raw/`, read-write on their own `/projects/` (including the `metadata/` subfolder). Operators (technicians) deposit data and never write to `/raw/` after their ingest finishes. **Person/role naming locked 2026-06-09:** registry `operator` column → **`researcher`** (who set up / ran the study); the equipment **operator** is recorded as both a registry column and the sidecar; permission roles renamed Operator→Tech, User→Researcher. | Distinguishes "who brings data in" from "who works with data after". Folder-level permissions stay clean (no file-level ACLs needed). See [11_OPERATIONS §1.1 + §2.1](11_OPERATIONS.md). |
| Subject/Sample identity (DRAFT, refines REG-01, 2026-06-03) | Model **Subject** (animal) and **Sample** (specimen) as distinct entities, each with its own id. `subject.facility_animal_id` = the facility canonical animal id `<animal_code>-AE-biomaGUNE-<NNNN>` **reused verbatim**; `sample_id` = the subject id for in-vivo (`organism`), or a within-subject specimen label for `tissue`; organ → structured `anatomical_entity`. **Option B:** in docs + sidecar; the registry projection columns (packed `subject_ids` + `anatomical_entity`) landed LIVE at the 2026-06-10 restart. | Grounded in FAIR/ISA/REMBI/BIDS/XNAT (all separate subject from sample; BIDS `participant_id` vs `sample_id`; FAIR "reuse existing identifiers, don't overload IDs with meaning"). Reusing the facility id makes it the DB join key for free. Supersedes the earlier bare `<short_project>_<short_sample>` composite. Open (REG-01/META-08) — a Data Office decision (user input welcome; this project has no PI sign-off gate). See [06_REGISTRIES §2.3](06_REGISTRIES.md). |
| Sample type vocabulary (DRAFT) | 5-value controlled vocab: `tissue` / `organism` / `cells` / `material` / `phantom` | REMBI-aligned: `sample_type` is the *kind of biological material*, not the species or anatomy. Covers everything in current scope and the foreseeable future. AxioScan template pre-fills `tissue`. Status open (REG-07) — a Data Office decision (user input welcome; no PI sign-off gate). The species (`sample_organism`) and anatomy (`anatomical_entity`) columns now exist in the live 28-column schema; remaining work is tracked in [tasks/STATUS.md](../tasks/STATUS.md). See [06_REGISTRIES §2.4](06_REGISTRIES.md). |
...
| ✅ **Operator GUI — `gjesus3_ingest.exe`** (DECIDED, shipped 2026-06-24) | ONE Flask app, two pages over the shared validated pipeline: microscopy `/` (recipes/builder) + a deliberately-simple MRI `/mri` (pull-from-scanner → preview → ingest, no recipes). Frozen to a **single ~95 MB Windows `.exe`** (PyInstaller; `paramiko` hidden-import + bundled `ftp_mirror.py`) and deployed to `\\GJESUS3\gjesus3\gjesus3-data\tools\` with shortcuts (`MRI Ingest` = exe `--mri`) + HTML guides. *(The old name `microscopy_ingest.exe` is retired — it is now one combined `gjesus3_ingest.exe`.)* **MRI source = SFTP pull, READ-ONLY** (only `listdir/stat/get` against `kenia`; no write path — audited); creds from `~/.ssh/gjesus3_mri.cred` (per-machine); staging to `<NAS>/staging` is invisible + auto-deleted after a clean ingest; `instrument_model` auto-derived from `acqp ACQ_station`; **destination project nameable from `${discovered.*}` metadata labels, resolved per scan (2026-09-04)**, so one run can file scans into several projects; NAS validated before pull; link-collision + overwrite warnings; default-on Dicomifier regen (graceful). | Non-technical adoption: a one-click tool that does the right thing with the fewest choices. Reuses the proven pipeline (no reimplementation). Read-only SFTP guarantees the acquisition console is never touched. See [10_TOOLS §5.2](10_TOOLS.md), [09_MODALITIES §1.4](09_MODALITIES.md) (`mri_scanner_model`), [`tools/operator/gui/README.md`](../tools/operator/gui/README.md). |

---

## Documentation Integrity Mirrors

Several specs are **mirrors of code** and must be kept in sync field-for-field (see [CONTRIBUTING-docs.md](../CONTRIBUTING-docs.md) for the full rule set): **[06_REGISTRIES](06_REGISTRIES.md)** ↔ the `USER_CONTROLLABLE_COLUMNS` / `AUTO_COLUMNS` sets in [`tools/ingest/resolver.py`](../tools/ingest/resolver.py) + `REGISTRY_FIELDS` in [`tools/ingest/registry.py`](../tools/ingest/registry.py); **[09_MODALITIES](09_MODALITIES.md)** "Auto-discovered fields" ↔ `EXPOSED_FIELDS` in the matching `tools/ingest/<eco>_metadata.py` (`czi_metadata.py`, `paravision_metadata.py`, `ni_metadata.py`); **[10_TOOLS](10_TOOLS.md)** YAML ingest schema ↔ [`tools/templates/ingest_template.yaml`](../tools/templates/ingest_template.yaml) + every per-instrument template under [`tools/templates/instruments/`](../tools/templates/instruments/). **Change a field in one, change it in all.**

**Contributor & agent rules:** see [CLAUDE.md](../CLAUDE.md) (agent operating rules) and [CONTRIBUTING-docs.md](../CONTRIBUTING-docs.md) (doc-architecture, status markers, cross-reference mirror rules, style).
...
- [x] ~~**Preclinical `subject:` block not yet auto-populated**~~ — **Phase 3 writer SHIPPED 2026-06-03** (non-blocking): `tools/ingest/enrichment.py` auto-populates `subject:` from the animal-facility DB (`tools/animal_db.py`) at ingest Step 8.4, with the §4.4.6 deferred-recovery pending list (`registries/pending_subject_metadata.csv`) + superuser `tools/recover_subject_metadata.py` for DB-miss / no-credentials. Spec in [08_METADATA §4.4](08_METADATA.md). The registry projection landed at the **2026-06-10 restart** — the packed `subject_ids` Auto column is now LIVE in the 28-column schema; remaining historical back-fill is tracked in [tasks/BACKLOG.md](../tasks/BACKLOG.md) (Phase 4).
- [x] ~~**`anatomy:` block writer not yet implemented**~~ — **Phase 3 writer SHIPPED 2026-06-03** (non-blocking): `tools/ingest/enrichment.py` writes the `anatomy:` block for `sample_type = organism` at ingest Step 8.4; `is_whole_body` highly-recommended tri-state (§4.7) + UBERON `region`, operator-entered via the top-level `anatomy:` YAML block (`mri_bruker` + `molecubes_ni` templates). Spec in [08_METADATA §4.6](08_METADATA.md). The registry `anatomical_entity` Auto column landed at the **2026-06-10 restart** and is now LIVE; remaining historical back-fill is tracked in [tasks/BACKLOG.md](../tasks/BACKLOG.md) (Phase 4). New META-09.
- [x] ~~**Preclinical `condition:` block writer not yet implemented**~~ — **Phase 3 writer SHIPPED 2026-06-03** (non-blocking): `tools/ingest/enrichment.py` writes the `condition:` block for `sample_type ∈ {organism, tissue}` at ingest Step 8.4; `is_control` highly-recommended tri-state (§4.7) + `disease_model` + `disease_state`, operator-entered via the top-level `condition:` YAML block (`mri_bruker` / `molecubes_ni` / `axioscan7` templates), `disease_model` auto-seeds from DB `projects.name`. Spec in [08_METADATA §4.5](08_METADATA.md). Live since the **2026-06-10 restart**; remaining historical back-fill of pre-restart acqs is tracked in [tasks/BACKLOG.md](../tasks/BACKLOG.md) (Phase 4).
...
> **🕗 PLANNED / DEFERRED:** Designed but not yet deployed — see [tasks/BACKLOG.md](../tasks/BACKLOG.md)
...
The full dated history now lives in the single repo changelog — see [CHANGELOG.md](../CHANGELOG.md). The numbered docs in this folder carry the current state; CHANGELOG.md carries the narrative of how we got here. Index-level milestones:

| Date | Change |
|------|--------|
| 2026-07-20 | **Finder refresh reworked.** The wholesale rebuild-on-every-ingest (CLI-only — so operator GUI ingests silently never refreshed the index) is replaced by a scheduled global rebuild + a targeted per-project refresh on ingest; CLI opt-in via `--refresh-index` ([tools/FINDER.md](../tools/FINDER.md), [10_TOOLS pipeline step 14](10_TOOLS.md)). |
| 2026-07-16 | **MRI no-DICOM backfill executed + made official policy.** The 612-row regen worklist drained (153 regenerated in place / 365 not-applicable / 94 no-source); new [10_TOOLS §3.8](10_TOOLS.md) (backfill tool, worklist status domain, `registry.update_row`) + [11_OPERATIONS §5.5](11_OPERATIONS.md) (data-office procedure); the false re-run-fills-placeholders claim corrected everywhere; operator runbook archived to `tasks/archive/`. |
...
| 2026-06-23 | Researcher Finder enhancements (`registries/index.html` auto-refresh). |
| 2026-06-10 | True-production restart (quasi-prod purged; fresh 28-column registry schema). |
