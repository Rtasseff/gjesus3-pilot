...
**Status:** ✅ **TRUE PRODUCTION** (active since the 2026-06-10 restart). Live scale: ~13,555 acquisitions in `/raw/`, ~50 projects (each hard-linked to its raw data), ~716 subjects in `registry_subjects.csv`; `publications/` deferred (empty).
**Last Updated:** 2026-06-26 — **public-release documentation refactor** (status promoted to TRUE PRODUCTION; settled decisions marked ✅ DECIDED; added the System Lifecycle note + the Documentation Integrity Mirrors and Contributor-rules pointers; clarified EM as reserved/not-deployed; operator GUI corrected to the single frozen `gjesus3_ingest.exe`). Prior: 2026-06-24 — **operator GUI rollout** (`gjesus3_ingest.exe`: the microscopy GUI + a simple **MRI page** `/mri`, frozen to one Windows `.exe` deployed at `\\GJESUS3\gjesus3\gjesus3-data\tools\` with shortcuts + HTML guides; MRI source = **read-only SFTP pull** from `kenia`, `instrument_model` auto-derived from `ACQ_station`, staging auto-deleted, NAS-validated-before-pull — [10_TOOLS §5.2](10_TOOLS.md), Key Decisions). Prior: 2026-06-23 — **researcher Finder enhancements** (auto-refresh of `registries/index.html` at the end of each successful ingest (non-fatal); expanded table columns incl. project `short_name` + owner; detail-panel File size + Project description with Copy-path buttons; `DEFAULT_LINK_BASE` corrected to the container UNC `\\GJESUS3\gjesus3\gjesus3-data`; `index.html` documented as a generated artifact — [06_REGISTRIES §1.2](06_REGISTRIES.md), [`tools/FINDER.md`](../tools/FINDER.md)). Prior: 2026-06-15 — **2026-06 code-review correction pass merged to `main`** (extractor resume-safety + `--force`, NI/MRI copy collision-guards, registry-lock stale-break TOCTOU, pre-flight registry-header check, operator/parsing/atomic-write fixes) **+ anatomy auto-derive + back-fill (MRI + microscopy)** (operator runbook [`tools/ANATOMY_BACKFILL.md`](../tools/ANATOMY_BACKFILL.md); UBERON ids OLS-verified — 08_METADATA §4.6.4) **+ true-production restart** (2026-06-10: quasi-prod purged; fresh registry schema adds the `sample_organism` + `subject_ids` + `anatomical_entity` Auto columns) **+ the NI multi-animal work** (one-row-per-subject `registry_subjects.csv`; the AUTO column `subject_id` → packed **`subject_ids`** rename DONE 2026-06-12). **Full dated history → [CHANGELOG.md](../CHANGELOG.md).**
