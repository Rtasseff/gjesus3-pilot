...
| Pending list — subjects | `/gjesus3/registries/pending_subject_metadata.csv` | Deferred-recovery queue for DB-miss / no-credentials subject enrichment (§2.3.2; [08_METADATA §4.4.6](08_METADATA.md)). |
| Pending list — DICOM regen | `/gjesus3/registries/pending_dicom_regen.csv` | Deferred-recovery queue for no-DICOM ParaVision MRI exams ingested where Dicomifier was unavailable; a later Linux/WSL pass re-pulls from the platform + regenerates ([10_TOOLS](10_TOOLS.md); `tools/ingest/pending_dicom.py`). |
| Pending list — NI recon fill | `/gjesus3/registries/pending_ni_recon.csv` | **Deferred-fill queue for NI acquisitions registered before their DICOMs exist** (Molecubes reconstruction lags acquisition — a recon dir exists but is empty at sync time). Each is registered as a placeholder (empty `<ACQ-ID>.data/`, `file_count=0`) and queued here; a later sync re-finds the recon at its box path (the NI source is never deleted) and fills the `.data/`, flipping `status` → `filled`. One acquisition per reconstruction. Schema: `acq_id·original_name·recon_index·session_id·canonical_path·ingest_config·queued_datetime·status` (`tools/ingest/pending_ni_recon.py`). |
| Pending list — project links | `/gjesus3/registries/pending_links.csv` | **Deferred-recovery queue for project hard links that couldn't be created** because the ingesting machine's mount has no hard-link support (the NI Mac over SMB → `os.link` `ENOTSUP`). The acquisition is fully registered; only the `<project>/raw_linked/` link is deferred (a `.PENDING-LINK.txt` stand-in marks the spot). A later [`tools/relink_pending.py`](../tools/relink_pending.py) pass on a hard-link-capable machine (the Windows workstation) creates the real links and flips `status` → `linked`. Schema: `acq_id·project_id·link_name·raw_primary_canonical·primary_kind·reason·host_os·queued_datetime·status`. Mirrors the subjects/DICOM pending pattern (`tools/ingest/pending_links.py`). |
