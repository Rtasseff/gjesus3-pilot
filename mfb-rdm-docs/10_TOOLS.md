...
| `per_recon_acquisitions` | `false` | New 2026-06-25 (NI-live, opt-in). When `true` AND `copy_strategy: ni_molecubes`, the discovery fan-out (`config.fanout_ni_recons`) splits each Molecubes anchor (`<ts>_<MODALITY>/`) into **one acquisition per `recon_<idx>/`** that has DICOMs — its own ACQ-ID, registry row, `<ACQ-ID>.data/`, and project link (`original_name = <anchor>/recon_<idx>`, the stable per-recon dedup key; recon indices are append-only on the box). A recon dir with **no DICOMs yet** (reconstruction still running) is **skipped + logged**, not failed, and picked up on a later sync; a reconstruction added later is automatically a new acquisition. Set ONLY in `molecubes_ni_live.yaml`; without it (archive NI, microscopy, MRI) the anchor stays one bundled acquisition (unchanged). The `link_filename` must include `${discovered.ni_recon_idx}` to stay unique across a scan's recons. |
...
- **Enrichment-projection columns (added 2026-06-10): `sample_organism`, `subject_ids`, `anatomical_entity`** (the `subject_id` column was **renamed to packed `subject_ids` 2026-06-12, NI-LIVE-08** — `;`-joined, always-a-list) — also auto-populated, but sourced from the non-blocking enrichment blocks rather than the analysis step: `registry.build_row` projects them from the Step 8.4 `subject:` / `anatomy:` blocks (`subject.species` → `sample_organism`, `subject.facility_animal_id` → `subject_ids`, `anatomy.region.label` → `anatomical_entity`). Set the upstream values via `auto_discover.subject_from_db` / `subject_lookup:` (DB) and the `anatomy:` block — **never as `registry:` keys** (the resolver rejects them there). Blank for non-animal samples. See [06_REGISTRIES §2](06_REGISTRIES.md), [08_METADATA §4.4/§4.6](08_METADATA.md).
...
### 3.8 `backfill_dicom_regen` — deferred DICOM regeneration backfill (MRI)

**Purpose:** Data-office backfill that **fills the empty `<ACQ-ID>.data/` placeholders** of no-DICOM ParaVision MRI acquisitions from staged platform sources, **keeping each acquisition's existing ACQ-ID**. **Location:** `tools/backfill_dicom_regen.py`. This is the deferred-recovery pattern of §3.7 (`recover_subject_metadata`) applied to pixel data — a controlled in-place **update** of an already-registered acquisition, deliberately **not** a re-ingest: the ingest dedup keys on `(acq_date, original_name)` presence in the registry and skips every registered placeholder, and bypassing the dedup would mint a *new* ACQ-ID and append a *duplicate* registry row (the ACQ-ID is allocated inside `ingest_single`, downstream of the dedup). Design decided 2026-07-16; see `CHANGELOG.md`.

**The worklist it drains — `registries/pending_dicom_regen.csv`** (`tools/ingest/pending_dicom.py`; queued automatically by the ingest since 2026-06-24 whenever a no-DICOM MRI exam is registered with an empty placeholder). Status domain:

| `status` | Meaning | Terminal? |
|---|---|---|
| `pending` | An image exam awaiting a Dicomifier pass — **the drain set.** | no |
| `regenerated` | The backfill filled its `.data/` (registry row + sidecar updated, verified). | yes |
| `not-applicable` | Spectroscopy / calibration (`nonimage_marker` = STEAM/PRESS/WOBBLE); image regeneration can never apply. Input set for the separate spectroscopy path (`tasks/BACKLOG.md`). | yes |
| `no-source` | An **image** exam whose platform source retains no reconstructable data — no `pdata/<idx>/2dseq` anywhere (header-only, or `fid`-only raw k-space). A data-loss record, not a task. Set only by the explicit, human-gated `--mark-no-source` pass. | yes |

**What one drained exam gets** (in order, with verify-after-write + rollback): DICOMs regenerated via the same `paravision_regen` path an ingest-time regen uses (Dicomifier + all three workarounds — see the `auto_regenerate_dicom` row in the §2.1 `ingest:` flag table) into the existing `.data/`; full re-hash + PixelSpacing validation; `checksums.json` rebuilt; blank-only sidecar `subject.age_at_acquisition` fill (from `date_of_birth` + the regenerated DICOM's StudyDate); a 3-line `README.txt` refresh (original registration date preserved); then — inside **one** `registry_lock` critical section — the **registry row updated in place** via `registry.update_row` (`file_count`, `file_size_mb`, `checksum_present`; blank-only `acquisition_datetime` + `file_format`) and the worklist row flipped to `regenerated`. `registry.update_row` (added 2026-07-16) is the only in-place registry write path in the toolset: header-checked read-all/atomic-rewrite matched on `acq_id`, with `only_if_blank` controlled-write semantics — it never overwrites a populated value.

**Safeguards** (mirroring §3.7): dry-run by default (`--apply` to write); idempotent (terminal rows skipped; a populated-but-still-`pending` row is *reconciled* — verified and book-kept from disk, never regenerated over); `no-source` is never auto-inferred from a failed attempt; per-exam error isolation.

```bash
# Preview (safe anywhere; no Dicomifier needed):
PYTHONPATH=tools python tools/backfill_dicom_regen.py

# Regeneration itself runs from WSL (see 11_OPERATIONS §5.5 for the full procedure):
PYTHONPATH=tools python tools/backfill_dicom_regen.py --apply --limit 1   # pilot one exam
PYTHONPATH=tools python tools/backfill_dicom_regen.py --apply            # drain

# Dedicated, human-gated pass recording the un-regenerable rows (no Dicomifier needed):
...
operators run it from there via desktop shortcuts (no Python/admin install). Updating = drop
a freshly-built exe in that folder. The CLI tools still run from the repo checkout on the
data-office machine.
...
### 5.2 Operator GUI — `gjesus3_ingest` (DECIDED, shipped 2026-06-24)

A local browser-based front-end for non-technical operators, over the SAME validated
pipeline (it never reimplements ingest — every preview/commit goes through the shared
`tools/operator/` core → `ingest_raw`). **One Flask app, two pages:**

| Page | Route | For | Shape |
|------|-------|-----|-------|
| **Microscopy** | `/` | ZWSI / CELL / LSM9 operators | Recipe-runner (pick a saved recipe → point at a folder → preview → ingest) + a Builder to define a new convention. See `tools/operator/README.md`. |
| **MRI** | `/mri` | Researchers who run the Bruker scanner | Deliberately simple — ONE locked convention (the `mri_bruker` template), no recipes/builder. Pull from the scanner → preview → ingest. |

Both reuse the shared plumbing (NAS-root handling, the in-page folder browser, the SSE
ingest-log stream, the core pipeline). The microscopy page is unchanged by the MRI work
apart from a cross-link + a behaviour-identical refactor.

**MRI page — design decisions (DECIDED 2026-06-24):**

- **Source = SFTP pull from the acquisition console; READ-ONLY.** The page lists the
  ParaVision study folders on the host (`kenia`, the two PV roots in
  [equipment/historical_data_archives.md](../equipment/historical_data_archives.md) §MRI),
  filtered to the MFB `jrc` convention, newest-first; the operator ticks studies and pulls
  them. The ONLY operations ever issued against the host are `listdir_attr` / `stat` / `get`
  — there is **no code path that can write, modify, or delete** anything on the scanner
  (audited). Engine: `tools/ftp_mirror.py::mirror` (download-only, `.part`+atomic-rename,
  idempotent). Optional defence-in-depth: a server-side read-only `mriuser` account.
- **Credentials** resolve from `~/.ssh/gjesus3_mri.cred` (INI `[mri]` host/user/password;
  `GJESUS3_FTP_*` env vars override) — a per-machine deploy step, never in the repo/bundle.
- **Staging is invisible + ephemeral.** Pulled studies land in `<NAS>/staging/<auto-batch>`
  (keeps the follow-on ingest NAS-local) and the batch is **auto-deleted after a
  fully-successful real ingest** (guarded so it can only ever remove a batch subfolder, never
  a NAS top-level dir / drive root). The operator never sees or manages a staging path; there
  is no manual "source folder" field.
- **`instrument_model` is auto-derived** from `acqp ACQ_station` (the literal Bruker system
...
- **No-DICOM deferral.** Dicomifier isn't packaged for Windows (conda-forge skips it; not on
  ...
  **queued to `registries/pending_dicom_regen.csv`** (`tools/ingest/pending_dicom.py`) recording
  ...
  version. A later data-office pass — **`tools/backfill_dicom_regen.py` (§3.8)**, run from a
  Dicomifier-equipped WSL box against sources staged from `kenia` — regenerates and fills the
  `.data/` **in place, keeping the ACQ-ID**. (Corrected 2026-07-16: this deferred fill is an
  in-place *update*, NOT a re-ingest — a re-run of the ingest dedupe-skips every registered
  placeholder, and bypassing the dedup would mint a duplicate ACQ-ID. See §3.8.) See the
...
**Freeze + distribution (DECIDED 2026-06-24).** Frozen to a single self-extracting Windows
`.exe` via `tools/operator/gui/gjesus3_ingest.spec` (PyInstaller; `paramiko` is a hidden
import — lazy-loaded — and `ftp_mirror.py` is bundled as data). **Build OUTSIDE OneDrive**
(it locks the build artifacts). Deployed to **`\\gjesus3\gjesus3\gjesus3-data\tools\`**:
`gjesus3_ingest.exe` + two UNC shortcuts (`Microscopy Ingest` → exe; `MRI Ingest` → exe
...
| **Shipped 2026-06-24** | Operator GUI (`gjesus3_ingest` — microscopy + MRI pages, frozen .exe on the NAS, §5.2) | Done |
| **Post-pilot** | user-supplied metadata workflows | Based on demand |
