...
|------|----------|-------------|
| Primary data file | ✅ Yes | Exactly one (see Section 4). For DICOM: a compressed archive (`.zip`/`.tar.gz`) named `<ACQ-ID>.zip` |
...
**Primary-file naming rule (full-mode ingest):** the primary file is renamed to a canonical name on copy:
- DICOM → `<ACQ-ID>.zip` (compressed archive)
- Microscopy single-file → `<ACQ-ID>.<ext>` (e.g. `<ACQ-ID>.czi`)

The original filename is preserved in the registry's `original_name` column and in `metadata.json`'s `user_supplied.original_name`, so the user-facing name from the source system is never lost.
