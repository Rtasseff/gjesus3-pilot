...
- **[`05_PROJECTS.md §2a`](mfb-rdm-docs/05_PROJECTS.md) ↔ `tools/ingest/project_naming.py` + `create_project.py` (INTEGRITY MIRROR).** The project reference model: the two identifiers (`project_id` / `name`), the case-insensitive resolution rule, and the invariant **folder == name, verbatim**. `project_naming.py` is the ONE construction site for the name→folder rule; `create_project.py` is the only creator; everything else reads the stored `folder_location`. §2a carries the consumer table — extend it when a new consumer appears.
...
### One concept → one name → one owning section → one construction site

**A rule for any concept that crosses layers** (operator input → config → code → registry → folder → docs). Added 2026-08-02 after the project-naming refactor, which existed only because all four halves of this rule had been skipped:

1. **One name.** The concept gets a single word, used unchanged at every layer. The project concept had four (`project_hint` in the config *and* in the registry, `short_name` in the projects registry, "Project hint" in the GUI, a `proj-`-prefixed folder) — so nobody could tell they were the same thing.
2. **A name that is TRUE.** `project_hint` stored resolved `PROJ-XXXX` ids, not hints. A lying column name is not cosmetic: it caused two rewrites of the migration plan and nearly a needless 13,582-row value migration. If a rename makes the name inaccurate, that is a blocker, not a detail — a `project_name` column holding ids would have been the same bug with a new spelling.
3. **One owning spec section**, with a consumer table, that everything else cites — e.g. [`05_PROJECTS §2a`](mfb-rdm-docs/05_PROJECTS.md). Without an owner, vocabulary drifts per layer and no single document is wrong, so nothing gets fixed.
4. **One construction site in code.** Exactly one module derives the concept's canonical form; every other consumer *reads the stored value*. The folder rule had lived in three places (`create_project`'s prefix, `ingest_raw`'s `.lower()`, `collisions.py` re-deriving a path) and they had already drifted; it now lives only in `tools/ingest/project_naming.py`.

When you add a cross-layer concept, add its owning section here as a mirror row.

---

## Ingest configs

- **Per-instrument templates** live in `tools/templates/instruments/` (e.g. `axioscan7.yaml`, `cell_observer_cells.yaml`, `lsm900.yaml`, `mri_bruker.yaml`, `molecubes_ni.yaml`, `molecubes_ni_live.yaml`) — they lock in the instrument-specific patterns (filename parse, registry mapping, project-name convention, `link_filename:` default). Each template's **header comments must list every `discovered.*` field auto-populated for that instrument** as the operator's reference card. Copy and edit; never edit in place. Add a new template when bringing a new instrument online.
