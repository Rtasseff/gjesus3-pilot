...
- **A new worktree branch means a FRESH session — normally not this one.** We cut a worktree for most code changes (above), and **standard practice is that Ryan opens a new session in that worktree to do the work.** In the originating session your job is to cut the worktree and write the **handoff document** — not to implement. **Rationale (Ryan, 2026-09-04):** it keeps the planning/context session small, and lets the implementation run on a cheaper model.
  - **The handoff doc must carry the context the fresh session will not have.** It starts cold: it reads `CLAUDE.md` and `tasks/STATUS.md` and knows nothing of your thread. Everything you established by investigation — why the change is scoped this way, the `file:line` you already located, the dead ends and traps you already hit, what "done" looks like, and **how to verify it** — has to be written down or it is lost and gets re-derived at full cost. **Memory files do not close this gap**: an agent that doesn't load them never sees them. Write for a reader who is competent but has zero history.
  - **Suggest a model, and say why.** Name the tier you think the task needs (e.g. *"mechanical edit across 6 files, all located below — a smaller/faster model is fine"* vs *"needs judgement about which rows are safe to touch — keep it on a strong model"*). The aim is the fewest tokens that still get the job done.
  - **Where it goes:** `HANDOFF.md` at the root of the new worktree, left **untracked** — the fresh session starts there and cannot miss it, and it never reaches `main`. The durable record still goes to [`tasks/BACKLOG.md`](tasks/BACKLOG.md) / [`tasks/STATUS.md`](tasks/STATUS.md) / [`CHANGELOG.md`](CHANGELOG.md) as usual.
  - **This is the default, not a prohibition.** If you think there is real value in doing the work directly from the existing session — you already hold context that would be expensive to re-encode, the change is genuinely trivial, planning and doing are inseparable — **ask, and make the case.** Don't just proceed. And if work you *did* start inline grows past what was agreed (a build, a deploy, a widening blast radius), stop and hand off then.
