...
5. The **Finder** (`registries/index.html` — a self-contained searchable web page, plus a per-project `index.html` in each project folder) is kept current — the per-project `index.html` refreshes on ingest, the global one on a schedule — so the new acquisition is immediately searchable. See [`tools/FINDER.md`](../tools/FINDER.md).
...
- **Reference materials** ([`mri-platform/`](./mri-platform/)):
  ...
  - **No-DICOM exams** — when an exam arrives without DICOMs (researcher skipped Bruker's exporter), the ingest regenerates them from `2dseq`+JCAMP-DX via Dicomifier (in WSL, three image fixes applied), or registers an empty placeholder queued for the data-office backfill. Official procedures: [`10_TOOLS.md`](../mfb-rdm-docs/10_TOOLS.md) (flag §2.1 · backfill tool §3.8) + [`11_OPERATIONS.md §5.5`](../mfb-rdm-docs/11_OPERATIONS.md). (The original operator runbook is archived at `tasks/archive/mri_no_dicom_regeneration_runbook.md`.)
...
- **Reference materials** ([`nuclear-imaging/`](./nuclear-imaging/)):
  ...
  - **Subject/folder value standard — PROPOSAL** (md, 2026-08-19) — [`nuclear-imaging/subject_naming_standard.md`](./nuclear-imaging/subject_naming_standard.md). ❓ **EVALUATING**, awaiting a Data Office ruling. The platform fixed the folder *structure* but never specified the *values*, so the subject parser had to accept whatever researchers typed — which is how a per-reconstruction `r1` folder became "animal 1" and misattributed 15 PROJ-0056 acquisitions to rats born two years earlier. Proposes a strict format for the hand-typed levels and enforcement by **refusal** at the operator dry-run, rather than a better guesser. Forward-only: the `S:\gnuclear` historical archive is explicitly out of scope.
  - **Live-machine remote access** (md, 2026-08-06) — [`nuclear-imaging/live_machine_remote_access.md`](./nuclear-imaging/live_machine_remote_access.md). How we reach the Molecubes acquisition box at all: it is NAT'd behind its own router, so it dials **out** to the Data Office workstation and we ride that reverse SSH tunnel back in. Covers the architecture (landing pad inside WSL, because the workstation has no admin rights), the safety classification for a **live acquisition machine** (what persists vs what is process-scoped), what is verified vs still untested, the 5-second forwarder bug that masqueraded as an authentication failure, and removal steps. Motivated by **Gate-0** for live-box sync (`os.link` on the box's CIFS mount) and by the fact that `datapath.txt` — the entire evidence base for the sync-rules document above — currently costs a physical visit to refresh. The workstation half lives in the separate `WorkstationOps` repo (operation `molecubes-tunnel`); the operator field card lives on the NAS at `S:\gnuclear\2026\Jesus\Ryan\tunnel.txt`.
